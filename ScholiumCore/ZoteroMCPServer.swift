...
public actor ZoteroMCPServer {
    ...
    private func callTool(params: ZoteroMCPJSONValue?, access: ZoteroMCPAccess) async -> ZoteroMCPJSONValue {
        do {
            ...
            case "zotero_inventory":
                execution = .success(try await inventory(arguments))
            case "zotero_collections":
                execution = .success(try await collections(arguments))
            case "zotero_tags":
                execution = .success(try await tags(arguments))
            case "zotero_groups":
                execution = .success(try await groups())
            case "zotero_children":
                execution = .success(try await children(arguments))
            case "zotero_fulltext":
                execution = .success(try await fullText(arguments))
            case "zotero_file_url":
                execution = .success(try await fileURL(arguments))
            case "zotero_export_bibtex":
                execution = .success(try await exportBibtex(arguments))
            case "zotero_citations":
    ...
    private func inspectItem(_ arguments: [String: ZoteroMCPJSONValue]) async throws -> ZoteroMCPJSONValue {
        ...
        if let raw = try? JSONDecoder().decode(ZoteroMCPJSONValue.self, from: match.response.body),
            let rawObject = raw.objectValue
        {
            // The version is the optimistic-concurrency token required by
            // zotero_update_item. Returning Zotero's data object also avoids
            // forcing an Agent to reconstruct fields from a lossy projection.
            result["version"] = rawObject["version"] ?? .null
            result["data"] = rawObject["data"] ?? .object(rawObject)
        }
    ...
    func selectedLibraryRoutes(_ selector: String?) async throws -> [LibraryRoute] {
        let routes = try await libraryRoutes()
    ...
    func selectedTarget() async throws -> SelectedTarget {
    ...
    func libraryRoutes() async throws -> [LibraryRoute] {
    ...
    private static func toolDefinitions(for access: ZoteroMCPAccess) -> [ZoteroMCPJSONValue] {
        switch access {
        case .readOnly:
            return toolDefinitions.filter { tool in
                tool.objectValue?["annotations"]?.objectValue?["readOnlyHint"]?.boolValue == true
            }
        case .full:
...
struct SelectedTarget: Sendable {
    let libraryID: Int
...
enum ZoteroMCPRequestFactory {
    enum APIResource {
        ...
        case topCollections(query: [URLQueryItem])
        case tags(query: [URLQueryItem])
        case searches
        case items(query: [URLQueryItem])
        case topItems(query: [URLQueryItem])
        case item(itemKey: String)
        case children(itemKey: String)
        case annotationChildren(itemKey: String)
        case fullText(itemKey: String)
        case fullTextVersions
    ...
    static func api(route: LibraryRoute, resource: APIResource) -> URLRequest? {
        ...
        case .fullText(let key):
            guard validKey(key) else { return nil }
            suffix = "/items/\(key)/fulltext"
            query = []
        case .fullTextVersions:
            suffix = "/fulltext"
            query = [URLQueryItem(name: "since", value: "0")]
    ...
    static func globalAPI(path: GlobalAPIPath) -> URLRequest? {
        var components = URLComponents()
        components.scheme = "http"
        components.host = "127.0.0.1"
        components.port = 23119
    ...
    static func apiWrite(
        ...
        components.host = "127.0.0.1"
        components.port = 23119
        components.path = route.apiPrefix + "/items/\(itemKey)"
        guard let url = components.url else { return nil }
        var request = URLRequest(url: url)
    ...
    private static let allowedQueryNames: Set<String> = [
        "format", "itemType", "q", "qmode", "limit", "itemKey", "include", "style",
        "sort", "direction", "start", "since",
    ]
...
enum GlobalAPIPath: String {
    case root = "/"
    case schema = "/schema"
    case itemTypes = "/itemTypes"
    case itemFields = "/itemFields"
    case creatorFields = "/creatorFields"
}
