...
public enum ZoteroMCPTransportLocator {
    /// Locates the first-party transport executable without launching it.
    public static func report(
        ...
        if let path {
            return ZoteroMCPTransportReport(
                ...
                note: "The bundled Scholium Zotero connection helper is present; live Zotero availability remains a separate observation."
            )
        }
        return ZoteroMCPTransportReport(
            descriptorID: descriptor.identifier,
            state: .notConfigured,
            note:
                "The bundled Scholium Zotero connection helper is unavailable. Reinstall Scholium to restore it."
        )
    }

    /// Performs an explicit MCP lifecycle probe against the bundled stdio
    /// command. The probe sends only `initialize` and
    ...
    public static func probe(
        ...
        // them to /dev/null so a verbose server cannot fill an
    ...
    /// discovery operation only; it never starts the helper or reads Zotero.
