    ...
    public static func read(_ data: Data, filename: String, request: AgentAttachmentRead) throws -> AgentAttachmentContent {
        func invalid(_ message: String) -> ScholiumMCPFailure {
            .init(
                ...
                recovery: "Choose an available text/image file or one valid PDF page and read its exact current revision.")
        }
        guard data.count <= 20 * 1_024 * 1_024, request.startUTF8 >= 0, (1...65_536).contains(request.maximumUTF8),
            request.startUTF8 == 0 || request.expectedFingerprint != nil
        else { throw invalid("The attachment read or continuation exceeds its permitted bounds.") }
        ...
        if isPDF {
            ...
            pdf = document
            page = selected
        } else if request.page != nil {
            throw invalid("A page locator applies only to a PDF.")
        }
        switch request.mode {
        case .text:
            let source: Data
            if let page {
                source = Data((page.string ?? "").utf8)
            } else {
            ...
            guard let text = NoteDocument.decodeUTF8PreservingBOM(source.subdata(in: request.startUTF8..<end)) else {
                throw invalid("The text slice is unreadable.")
            }
            return .init(
                filename: filename, fingerprint: fingerprint, kind: isPDF ? "pdf_text" : "utf8_text", page: request.page,
        ...
        case .image:
            ...
            if let page {
                ...
                guard bounds.width.isFinite, bounds.height.isFinite, bounds.width > 0, bounds.height > 0,
                    let rendered = page.thumbnail(of: CGSize(width: 1_024, height: 1_024), for: .cropBox).cgImage(
                        forProposedRect: nil, context: nil, hints: nil),
                    rendered.width <= 1_024, rendered.height <= 1_024
                else { throw invalid("The PDF page cannot be rendered within image bounds.") }
                image = rendered
            } else {
                guard let source = CGImageSourceCreateWithData(data as CFData, nil), CGImageSourceGetCount(source) == 1,
                    ...
                    let rendered = CGImageSourceCreateThumbnailAtIndex(
                        source, 0,
                        [
                ...
                else { throw invalid("The image is unreadable, multi-frame or outside supported image bounds.") }
                image = rendered
            }
            var bytes = Data()
            while true {
                ...
                guard let destination = CGImageDestinationCreateWithData(encoded, UTType.png.identifier as CFString, 1, nil) else {
                    throw invalid("Image encoding is unavailable.")
                }
                ...
                guard edge > 0, let source = CGImageSourceCreateWithData(bytes as CFData, nil),
                    let reduced = CGImageSourceCreateThumbnailAtIndex(
                        source, 0,
                        [
                            kCGImageSourceCreateThumbnailFromImageAlways: true,
                            kCGImageSourceThumbnailMaxPixelSize: edge, kCGImageSourceShouldCache: false,
                        ] as CFDictionary)
                else {
            ...
            return .init(
                filename: filename, fingerprint: fingerprint, kind: isPDF ? "pdf_page_image" : "image", page: request.page,
