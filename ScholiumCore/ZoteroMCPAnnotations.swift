...
extension ZoteroMCPServer {
    func referenceValue(route: LibraryRoute, itemKey: String, kind: ZoteroReference.Kind = .item,
                        page: Int? = nil, annotationKey: String? = nil) throws -> ZoteroMCPJSONValue {
        let library: ZoteroLibraryIdentity = route.groupID.map(ZoteroLibraryIdentity.group) ?? .user
        let reference = try ZoteroReference(library: library, kind: kind, itemKey: itemKey,
                                            page: page, annotationKey: annotationKey)
        return .object(["library": route.value, "kind": .string(reference.kind.rawValue),
    ...
    func listAnnotations(_ arguments: [String: ZoteroMCPJSONValue]) async throws -> ZoteroMCPJSONValue {
        ...
        let fingerprint = try materialFingerprint(.object(["attachment": attachment.value,
            "annotations": .array(annotations.map(\.raw))]), route: route, response: response)
        ...
        return .object(["attachment_reference": try referenceValue(route: route, itemKey: attachmentKey, kind: .pdf),
    ...
    func readAnnotation(_ arguments: [String: ZoteroMCPJSONValue]) async throws -> ZoteroMCPJSONValue {
        guard Set(arguments.keys).isSubset(of: ["library", "attachment_key", "annotation_key", "expected_fingerprint"]),
              ...
              let key = ZoteroReference.normalizedKey(value) else { throw ZoteroMCPServiceError.invalidArguments }
        let (route, attachmentKey) = try materialScope(arguments)
    ...
    func materialScope(_ arguments: [String: ZoteroMCPJSONValue]) throws -> (LibraryRoute, String) {
        ...
        guard selector.hasPrefix("group:"), let id = Int(selector.dropFirst(6)), id > 0,
              selector == "group:\(id)" else { throw ZoteroMCPServiceError.invalidArguments }
    ...
    func fileAttachment(_ key: String, route: LibraryRoute) async throws -> AttachmentObservation {
        ...
        guard let object = value.objectValue, object["key"]?.stringValue == key,
              ...
              ["imported_file", "imported_url", "linked_file"].contains(data["linkMode"]?.stringValue ?? "") else {
            throw ZoteroMCPServiceError.invalidResponse
        }
    ...
    private func materialFingerprint(_ value: ZoteroMCPJSONValue, route: LibraryRoute, response: ZoteroMCPHTTPResponse) throws -> String {
        ...
        let bytes = try encoder.encode(ZoteroMCPJSONValue.object([
    ...
    private func annotationValue(_ record: AnnotationRecord, route: LibraryRoute, response: ZoteroMCPHTTPResponse,
                                 includeContent: Bool) throws -> ZoteroMCPJSONValue {
        var value: [String: ZoteroMCPJSONValue] = [
            ...
            "reference": try referenceValue(route: route, itemKey: record.attachmentKey, kind: .pdf,
    ...
    static let annotationListTool = tool(name: "zotero_list_annotations",
        description: "List bounded annotation locators for one exact PDF attachment; no selected text or comments. Continuation requires the listing fingerprint.",
    ...
    static let annotationReadTool = tool(name: "zotero_read_annotation",
...
private struct AnnotationRecord {
    ...
    init(_ raw: ZoteroMCPJSONValue, attachmentKey: String) throws {
        ...
        self.key = key; self.attachmentKey = attachmentKey; self.raw = raw; self.data = data
    }
    ...
    var page: Int? {
        guard let position = data["annotationPosition"]?.stringValue,
              ...
              let index = decoded.objectValue?["pageIndex"]?.intValue, index >= 0, index < Int.max else { return nil }
        return index + 1
    }
}
