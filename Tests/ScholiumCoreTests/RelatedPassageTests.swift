...
struct RelatedPassageTests {
    ...
    func paragraphs() throws {
        ...
        for passage in passages {
            let range = try #require(Range(NSRange(location: passage.range.utf16LowerBound,
    ...
    func noNoteFallback() throws {
        let vault = UUID(), doc = NoteDocument(relativePath: "freedom.md", rawContent: "# freedom\n\nAn unrelated paragraph.\n\n```\nfreedom\n```\n")
        let candidate = RelatedContentCandidate(note: .init(vaultID: vault, relativePath: doc.relativePath), vaultRole: .sourceCorpus,
            title: "freedom", fingerprint: doc.fingerprint, reason: .identityMention(.init(mentions: [])))
        let seed = RelatedContentSeedSnapshot(noteID: .init(vaultID: vault, relativePath: "Draft.md"), source: "freedom",
