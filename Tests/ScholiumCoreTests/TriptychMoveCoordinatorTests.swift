...
@Suite("Atomic Unclassified classification")
struct UnclassifiedClassificationCoordinatorTests {
    @Test("Classification preserves exact bytes and leaves the external original unchanged")
    func successfulClassification() async throws {
        let fixture = try Fixture()
        defer { fixture.remove() }
        let source = try await fixture.importSource("\u{FEFF}---\r\ntitle: 'A' # keep\r\n---\r\n正文\r\n")
        let commit = try await fixture.coordinator().classify(
            sourceRelativePath: source.relativePath,
            expectedRevision: source.fingerprint,
            destinationRelativePath: "Unicode/研究.md"
        )

        #expect(commit.destination.relativePath == "Unicode/研究.md")
        let destination = try await fixture.repository.load(relativePath: commit.destination.relativePath)
        #expect(destination.sourceBytes == source.sourceBytes)
        #expect(try Data(contentsOf: fixture.externalSource) == source.sourceBytes)
        do {
            _ = try await fixture.control.loadUnclassified(relativePath: source.relativePath)
            Issue.record("The classified Unclassified copy should have been removed")
        } catch {
            // Missing is the expected post-commit state.
        }
    }

    @Test("Create failure leaves the source and destination unchanged")
    func createFailure() async throws {
        let fixture = try Fixture()
        defer { fixture.remove() }
        let source = try await fixture.importSource("source\n")
        let coordinator = fixture.coordinator(
            faults: TriptychTransactionFaultPlan(points: [.beforeDestinationCreate])
        )
        await #expect(throws: TriptychTransactionError.self) {
            _ = try await coordinator.classify(
                sourceRelativePath: source.relativePath,
                expectedRevision: source.fingerprint,
                destinationRelativePath: "A.md"
            )
        }
        #expect(try await fixture.control.loadUnclassified(relativePath: source.relativePath).fingerprint == source.fingerprint)
        await #expect(throws: VaultRepositoryError.self) {
            _ = try await fixture.repository.load(relativePath: "A.md")
        }
    }

    @Test("Source-delete failure removes the newly created duplicate")
    func sourceDeleteFailureRollsBackDestination() async throws {
        let fixture = try Fixture()
        defer { fixture.remove() }
        let source = try await fixture.importSource("source\n")
        let coordinator = fixture.coordinator(
            faults: TriptychTransactionFaultPlan(points: [.beforeUnclassifiedRemoval])
        )
        await #expect(throws: TriptychTransactionError.self) {
            _ = try await coordinator.classify(
                sourceRelativePath: source.relativePath,
    ...
    @Test("Rollback failure preserves both copies and a durable recovery record")
    func rollbackFailureIsDuplicateSafe() async throws {
        let fixture = try Fixture()
        defer { fixture.remove() }
        let source = try await fixture.importSource("source\n")
        let coordinator = fixture.coordinator(
            faults: TriptychTransactionFaultPlan(
                points: [.beforeUnclassifiedRemoval, .beforeClassificationRollback]
            )
        )
        await #expect(throws: TriptychTransactionError.self) {
            _ = try await coordinator.classify(
        ...
        #expect(try await fixture.control.loadUnclassified(relativePath: source.relativePath).fingerprint == source.fingerprint)
        #expect(try await fixture.repository.load(relativePath: "A.md").fingerprint == source.fingerprint)
        #expect(try await fixture.recovery.pending().first?.files.count == 2)
    }

    @Test("An existing destination fails preflight without deleting the source")
    func duplicateDestination() async throws {
        let fixture = try Fixture()
        defer { fixture.remove() }
        let source = try await fixture.importSource("source\n")
        _ = try await fixture.repository.create(relativePath: "A.md", content: "existing\n")
        await #expect(throws: TriptychTransactionError.self) {
            _ = try await fixture.coordinator().classify(
                sourceRelativePath: source.relativePath,
                expectedRevision: source.fingerprint,
                destinationRelativePath: "A.md"
            )
        ...
        #expect(try await fixture.control.loadUnclassified(relativePath: source.relativePath).fingerprint == source.fingerprint)
        #expect(try await fixture.repository.load(relativePath: "A.md").rawContent == "existing\n")
    }
    ...
    private struct Fixture {
        ...
        init() throws {
            root = FileManager.default.temporaryDirectory
                .appendingPathComponent("Scholium-Classify-\(UUID().uuidString)", isDirectory: true)
            let works = root.appendingPathComponent("Works", isDirectory: true)
            let destination = root.appendingPathComponent("Analyses", isDirectory: true)
            ...
            recovery = try TriptychMutationRecoveryStore(
                storageURL: support.appendingPathComponent("Triptychs/\(triptychID.uuidString)")
            )
        }

        func remove() { try? FileManager.default.removeItem(at: root) }

        func importSource(_ content: String) async throws -> NoteDocument {
            try Data(content.utf8).write(to: externalSource)
            _ = try await control.bootstrap(vaultIDs: Dictionary(
                uniqueKeysWithValues: WorkspaceVaultSlot.allCases.map { ($0, UUID()) }
            ))
            let imported = try await control.importMarkdown(at: externalSource)
            return try await control.loadUnclassified(relativePath: imported.lastPathComponent)
        }

        func coordinator(
            faults: TriptychTransactionFaultPlan = .none
        ) -> UnclassifiedClassificationCoordinator {
            UnclassifiedClassificationCoordinator(
                triptychID: triptychID,
                control: control,
