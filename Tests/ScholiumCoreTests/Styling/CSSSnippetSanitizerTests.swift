...
struct CSSSnippetSanitizerTests {
    ...
    @Test(
        "Network, cascade escape, and HTML injection are rejected",
        arguments: [
    ...
    @Test(
        "Research signals cannot be restyled",
        arguments: [
    ...
    @Test(
        "Layout and executable properties are not part of the contract",
        arguments: [
            "p { position: fixed; }",
            "p { display: none; }",
    ...
    @Test(
        "Broad selectors cannot hide protected descendants through opacity",
        arguments: [
            "a { opacity: 0; }",
    ...
    @Test(
        "Zero font sizes cannot hide document links",
        arguments: [
            "a { font-size: 0; }",
    ...
    func projectsPublicCalloutSelectors() throws {
        ...
        #expect(
            result.livePreviewCSS.contains(
