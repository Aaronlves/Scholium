...
struct AgentChangeStoreTests {
    ...
    func moveEvidenceLifecycle() async throws {
        ...
        let primaryID = UUID(); let linkedID = UUID(); let vault = UUID(); let linkedVault = UUID()
        let source = Data("\u{feff}Exact unchanged primary\r\n".utf8)
        let before = Data("---\nsummary: 'keep'\n---\n[[Old|label]]{{Authored comment.}}\n".utf8)
        let after = Data("---\nsummary: 'keep'\n---\n[[New|label]]{{Authored comment.}}\n".utf8)
        let primary = AgentNoteMoveEffect(noteID: primaryID, role: .sourceCorpus,
        ...
        let linked = AgentNoteMoveEffect(noteID: linkedID, role: .topicKnowledge,
        ...
        let collision = AgentNoteMoveEffect(noteID: linkedID, role: .sourceCorpus,
            source: primary.destination, destination: primary.destination,
            beforeFingerprint: .init(data: before), afterFingerprint: .init(data: after), rewrittenOccurrences: 1)
        await #expect(throws: AgentChangeError.self) {
            try await store.prepare(operation: .move, noteID: primaryID, role: .sourceCorpus,
        ...
        let prepared = try await store.prepare(operation: .move, noteID: primaryID, role: .sourceCorpus,
        ...
        _ = try await store.confirm(id: prepared.id, observedAfterFingerprint: primary.afterFingerprint,
        ...
        _ = try await reopened.markUndone(id: prepared.id, restoredFingerprint: primary.beforeFingerprint,
        ...
        let file = fixture.root.appendingPathComponent("Triptychs/\(fixture.triptychID.uuidString)/agent-changes-v1/\(prepared.id.uuidString.lowercased()).json")
        ...
        primaryObject["future_authority"] = true; moveObject["primary"] = primaryObject; payload["move"] = moveObject
