...
struct AgentChangeStoreTests {
    ...
    func recordEvidenceBindings() async throws {
        ...
        let prepared = try await store.prepare(
            operation: .metadata, noteID: note, role: .topicKnowledge,
        ...
        await #expect(throws: AgentChangeError.self) {
            try await store.prepare(
                operation: .metadata, noteID: UUID(), role: .topicKnowledge,
                originalRelativePath: "Topic.md", finalRelativePath: "Topic.md", beforeData: before, afterData: after)
        }
        let vault = UUID()
        let old = DocumentAttachmentRecord(
            id: UUID(), noteID: note, vaultID: vault,
        ...
        await #expect(throws: AgentChangeError.self) {
            try await store.prepare(
                operation: .attachment, noteID: note, role: .topicKnowledge,
