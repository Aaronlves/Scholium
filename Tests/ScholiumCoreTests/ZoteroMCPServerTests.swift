...
struct ZoteroMCPServerTests {
    ...
    @Test("Full mode updates one exact item with an expected Zotero version")
    func fullModeUpdatesVersionBoundItem() async throws {
        let client = MockZoteroMCPHTTPClient()
        ...
        let response = try await rpc(
            ...
            params: [
                ...
                "arguments": [
                    "library": "user", "item_key": "ITEM0001",
                    "data": ["title": "After"], "expected_version": 7, "confirm": true,
                ],
            ], access: .full)
    ...
    @Test("Zotero writes fail closed before transport without confirmation or a current version")
    func writesRequireConfirmationAndVersion() async throws {
        let client = MockZoteroMCPHTTPClient()
        let server = ZoteroMCPServer(client: client)
    ...
    func frameParserSupportsBothModes() throws {
        ...
        #expect(
            frames == [
                ...
                MCPFrame(body: headerBody, mode: .contentLength),
            ])
    }
    ...
    private func rpc(
        ...
        id: Int,
        method: String,
        params: [String: Any], access: ZoteroMCPAccess = .readOnly
    ) async throws -> [String: Any] {
