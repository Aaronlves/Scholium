...
struct AgentChatObjectActionTests {
    @Test func controlsTrackObjectsWithoutInterceptingText() async throws {
        ...
        let window = NSWindow(contentRect: NSRect(x: 100, y: 100, width: 320, height: 650),
        ...
        let geometry = try #require(try await web.evaluateJavaScript("""
        ...
        for (tracker, rect) in zip(trackers, geometry) {
            let slot = CGRect(x: try #require(rect["left"]), y: try #require(rect["top"]),
            ...
            #expect(abs(native.minY - rendered.minY) < 2 && abs(native.height - rendered.height) < 2,
            ...
            #expect(hit === web || hit?.isDescendant(of: web) == true,
                "The native action layer must leave text selection with WebKit")
        }
        func identities() async throws -> [Int] {
            try #require(try await web.evaluateJavaScript("[...document.querySelectorAll('.scholium-reply-object')].map(e => Number(e.dataset.replyIdentity))") as? [Int])
        }
        ...
        #expect(replacedIDs.last != originalIDs.last,
    ...
    @Test func diagramFitsItsPreviewAcrossAppearanceAndFailureStates() async throws {
        for (source, width, height, dark, contrast) in [
            ...
            let host = NSHostingView(rootView: AgentChatDiagram(source: "```mermaid\n" + source + "\n```")
                .environment(\.colorScheme, dark ? .dark : .light))
            host.appearance = NSAppearance(named: contrast
            ...
            while ContinuousClock.now < deadline {
                window.layoutIfNeeded(); host.layoutSubtreeIfNeeded()
                if let web = reader(host), let measured = try? await web.evaluateJavaScript("""
                    (() => {
                    ...
                    """) as? [String: Any] { metrics = measured; break }
                try await Task.sleep(for: .milliseconds(20))
            }
    ...
    private func waitFor(_ text: String, in web: WKWebView) async throws {
        ...
        while ContinuousClock.now < deadline {
            if let found = try? await web.callAsyncJavaScript("return document.body.textContent.includes(text)",
