...
struct FrontendArchitectureTests {
    ...
    func compactLibraryComponentContract() throws {
        ...
        for section in ["Folders", "Integrity", "Content", "Properties"] {
            #expect(filterMenuSource.contains("Section(\"\(section)\")"))
        }
        #expect(filterMenuSource.components(separatedBy: "Section(\"Properties\")").count == 2)
        #expect(filterMenuSource.contains("Menu(\"Sort By\")"))
        #expect(filterMenuSource.contains("Menu(\"Property Field\")"))
    ...
    @Test("User Appearance changes reach the generated document CSS")
    func structuredAppearanceCSS() {
        var customized = DocumentAppearanceProfile(name: "Custom")
