...
struct FrontendArchitectureTests {
    ...
    func initialVaultPublicationReusesAcceptedSnapshot() throws {
        ...
        let acceptedSnapshot = try #require(
            storeSource.range(
    ...
    func featureViewsUseResolvedFunctionalColorRoles() throws {
        ...
        for (path, source) in viewSources.sorted(by: { $0.key < $1.key }) {
            ...
            if NativeSettingsSourceScope.paths.contains(path) {
                ...
                #expect(
                    rawFunctionalColor.numberOfMatches(
                        in: source,
    ...
    func compactLibraryReachability() throws {
        ...
        #expect(
            ScholiumWorkspaceToolbarController.itemIdentifiers == [
                ...
                .flexibleSpace,
                .space,
                ScholiumWorkspaceToolbarController.Item.notifications,
        ...
        #expect(!toolbarSource.contains("item.view = NSHostingView"))
        ...
        #expect(
            toolbarSource.contains(
                "appState.canToggleResearchInspector"
            ))
    ...
    func compactLibraryComponentContract() throws {
        ...
        let notificationsSource = try String(
            contentsOf: repository.appendingPathComponent(
                "Scholium/UI/Components/ScholiumWorkspaceToolbar.swift"), encoding: .utf8)
        #expect(notificationsSource.contains("private func refreshNotifications()"))
        #expect(!componentsSource.contains("title: \"ATTENTION\""))
        #expect(
            notificationsSource.contains(
                "notificationsPopover.show(relativeTo: item)"
            ))
        #expect(notificationsSource.contains("bell.badge"))
        #expect(
        ...
        let headerControlSource = try String(
            contentsOf: repository.appendingPathComponent(
        ...
        #expect(notificationsSource.contains("No notifications"))
        #expect(notificationsSource.contains("Checking Notifications"))
        #expect(notificationsSource.contains("Notifications Unavailable"))
        ...
        #expect(!sidebarSource.contains("TriptychNotificationsEntry("))
        ...
        #expect(!sidebarSource.contains("rectangle.compress.vertical"))
        #expect(!sidebarSource.contains("rectangle.expand.vertical"))
        #expect(workspaceNavigatorSource.contains("WorkspaceSegmentedControl"))
        #expect(workspaceNavigatorSource.contains("NSFont.systemFontSize"))
        #expect(workspaceNavigatorSource.contains("control.controlSize = usesAccessibilitySize"))
    ...
    func scholarlyEditorialWorkspaceSurfaceContract() throws {
        ...
        #expect(content.contains("ResearchSearchSurface("))
    ...
    func semanticTypographyContract() throws {
        ...
        #expect(appearance.headings.level2.scale == 1.22)
        #expect(appearance.headings.level3.scale == 1.14)
        #expect(appearance.headings.level4.scale == 1.08)
        #expect(appearance.headings.level5.scale == 1.02)
        #expect(appearance.headings.level6.scale == 0.98)
        ...
        while let sourceURL = enumerator.nextObject() as? URL {
            ...
            if NativeSettingsSourceScope.paths.union(NativeChatSourceScope.paths).union(NativeSidebarSourceScope.paths).contains(
                sourceURL.path.replacingOccurrences(of: repository.path + "/", with: ""))
            {
    ...
    func nativeGlassControlGeometry() {
        let swiftUIButton = NSHostingView(
            rootView: ScholiumInkIconControl(
                title: "Search",
    ...
    @Test("Search results retain native list selection and activation")
    ...
    @Test("Research Search has an inline field and a window-owned advanced surface")
    ...
    func structuredAppearanceCSS() {
        ...
        #expect(css.contains("--scholium-appearance-h2-before: 0.8em"))
        #expect(css.contains("--scholium-appearance-h3-after: 0.28em"))
        #expect(css.contains("--scholium-appearance-h6-align: start"))
        #expect(
            css.contains(
        ...
        #expect(css.contains("\"STFangsong\""))
        #expect(css.contains("\"FangSong\""))
        #expect(css.contains("margin-inline-start: 0em"))
        #expect(css.contains("margin-inline-end: 0em"))
        ...
        #expect(css.contains("font-family: \"Kaiti SC\", \"STKaiti\", serif"))
        #expect(
            ScholiumWebDesignTokens.documentPresentationCSS.contains(
                ".scholium-document p em :lang(zh-Hans)"
            )
        )
        #expect(
            ScholiumWebDesignTokens.documentPresentationCSS.contains(
        ...
        customized.settings.headings.level3.scale = 1.3
        customized.settings.body.cjkStrongFontFamily = "Noto Sans CJK SC"
        customized.settings.body.cjkEmphasisFontFamily = "LXGW WenKai"
