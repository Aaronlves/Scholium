...
struct MCPNoteDisplayTests {
    ...
    func exactWindowNavigation() async throws {
        ...
        let analyses = triptych.appendingPathComponent("01-analyses"), topics = triptych.appendingPathComponent("02-topics"), works = triptych.appendingPathComponent("03-works")
        let source = Data("\u{feff}# 原文\r\n\r\nExact **text** 😀.\r\n".utf8)
        let file = analyses.appendingPathComponent("Source.md")
        try source.write(to: file); try Data("# Peer\n".utf8).write(to: analyses.appendingPathComponent("Peer.md"))
        ...
        do {
            ...
            store.registerNoteDisplayWindow(id: window.nativeWindowID, window: .init(state: {
                .init(triptychID: configured.id, canDisplay: visible, visibleConversationID: nil)
            }, display: { target, admitted in didDispatch = true; try await window.displayAgentNote(target, admitted: admitted) }))
            var flushes = 0
            let router = MCPAppBridgeRequestRouter(runtime: store.applicationRuntime, flushEditors: { _ in flushes += 1 }, openTriptychs: { [configured.assignment] },
                displayWindows: { store.displayWindowValues(triptychID: $0) }, displayNote: { try await store.displayAgentNote(windowID: $0, target: $1, request: $2) })
            let range = try #require(source.range(of: Data("text".utf8)))
            let args: [String: MCPJSONValue] = ["triptych_id": .string(configured.id.uuidString), "window_id": .string(window.nativeWindowID.uuidString),
                "note_id": .string(id.uuidString), "expected_fingerprint": .object(["sha256": .string(DocumentFingerprint(data: source).sha256), "byte_count": .integer(source.count)]),
            ...
            visible = true
            var wrong = args; wrong["expected_text"] = .string("invented")
            ...
            for cancel in [false, true] {
                ...
                else { window.enqueueDocumentTransition({}) }
                release?.resume()
            ...
            draftArgs["expected_fingerprint"] = .object(["sha256": .string(DocumentFingerprint(data: draftBytes).sha256), "byte_count": .integer(draftBytes.count)])
            ...
            session.suppressAutosave = true; session.editingSource = "Unsaved source"
            #expect(await router.handle(.init(tool: .showNote, arguments: args)).error != nil)
