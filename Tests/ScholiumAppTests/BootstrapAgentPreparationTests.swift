...
struct BootstrapAgentPreparationTests {
    ...
    func promptContract() {
        ...
        #expect(prompt.contains("host's project-level Skill mechanism"))
        #expect(prompt.contains("Do not substitute, rewrite, or scan for another Scholium Skill"))
        #expect(prompt.contains("Reload Skill discovery if the host requires it"))
        ...
        #expect(prompt.contains("never overwrite, merge, or shadow"))
        #expect(prompt.contains("Do not read Triptych research files or request a pairing code during setup"))
        #expect(prompt.contains("A later direct Run uses `scholium agent start`"))
        #expect(prompt.contains("Confirm the host discovers every returned Skill name"))
    }
