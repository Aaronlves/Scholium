...
struct BootstrapAgentPreparationTests {
    ...
    func promptContract() {
        ...
        #expect(prompt.contains("It may install only `scholium` and `Scholium_ScholiumCore.bundle`"))
        #expect(prompt.contains("Do not use `sudo`"))
        #expect(prompt.contains("edit PATH, shell profiles, global Agent configuration, or macOS quarantine metadata"))
        #expect(prompt.contains("`product=Scholium`"))
        #expect(prompt.contains("`cli_version=\(ScholiumProductIdentity.marketingVersion)`"))
        #expect(prompt.contains("Ignore other version fields"))
        #expect(prompt.contains("$HOME/.local/bin/scholium version --format json"))
        #expect(prompt.contains("`doctor --format json`"))
        #expect(prompt.contains("`help agent`"))
        ...
        #expect(prompt.contains("register every returned source as a project Skill"))
        ...
        #expect(prompt.contains("Do not begin research or pair during setup"))
        #expect(prompt.contains("Confirm the CLI and returned Skill names are available"))
        #expect(!prompt.contains("Project preparation:"))
        #expect(!prompt.contains("Reload Skill discovery"))
    }
