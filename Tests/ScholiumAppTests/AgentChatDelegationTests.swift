...
struct AgentChatDelegationTests {
    ...
    func continuity() async throws {
        ...
        #expect(reopened.selected?.messages.first { $0.id == record.id }?.activity?.delegation == report)
        #expect(
            reopened.selected?.messages.first { $0.activity?.delegation?.operation == .interacted }?
    ...
    func unreadable() throws {
        let activity = try #require(
            AgentChatActivityProjection.withLocalizedFailure(
                CodexChatActivity.parse(
                    [
                        "type": .string("collabAgentToolCall"), "status": .string("completed"),
                    ], completed: true, threadID: "parent")))
