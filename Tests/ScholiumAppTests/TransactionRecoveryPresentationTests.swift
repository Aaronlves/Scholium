...
struct TransactionRecoveryPresentationTests {
    ...
    func creationActionsAreConsequential() {
        let present = TransactionRecoveryActionPresentation(
            record: record(
        ...
        let uncertain = TransactionRecoveryActionPresentation(
            record: record(
                operation: .noteCreation,
                role: .createdNote,
                state: .unreadable
    ...
    func saveRecoveryUsesGenericCompletion() {
        let save = TransactionRecoveryActionPresentation(
            record: record(
                operation: .noteSave,
                role: .savedNote,
                state: .intendedBytesRemain
    ...
    func committedRefreshFailureUsesCommittedTruth() {
        #expect(
            TransactionRecoveryActionPresentation.committedRefreshMessage
