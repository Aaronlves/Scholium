...
struct AgentChatNotificationTests {
    ...
    func outcomes() async throws {
        ...
        let recorder = ChatNotificationRecorder()
        let controller = try await makeController(root, recorder: recorder)
        let first = try #require(controller.selectedID)
        controller.editDraft("duplicate-completion private-title")
        controller.send()
        ...
        controller.editDraft("fail-turn")
        controller.send()
    ...
    func pendingInput() async throws {
        ...
        let question = try #require(controller.approvals.first)
        controller.newConversation()
        ...
        #expect(recorder.events.count == 2)
        controller.editDraft("hold note-operation")
        controller.send()
        try await wait { controller.state == .working && controller.selected?.pendingMessageID == nil }
    ...
    func opening() async throws {
        ...
        let archived = try #require(controller.selectedID)
        let triptych = controller.triptychID
    ...
    func windowRouting() async throws {
        ...
        do {
            ...
            editor.suppressAutosave = true
            editor.editingSource = "Unsaved researcher argument"
