...
struct AgentChatTurnPresentationTests {
  ...
  func activeActivity() {
    func message(_ id: String, _ status: AgentChatActivity.Status, turn: String = "current") -> AgentChatMessage {
      var result = AgentChatMessage(id: id, role: .operation, text: "",
        activity: .init(kind: .read, status: status, source: .runtime))
      result.turnID = turn
      return result
    }
    let values = [message("running", .running), message("done", .completed),
  ...
  func statusPlacement() {
    ...
    for (history, owners) in [([request], ["request"]), ([request, addition], ["addition"]),
