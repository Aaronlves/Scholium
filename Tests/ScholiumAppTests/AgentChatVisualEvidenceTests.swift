...
struct AgentChatVisualEvidenceTests {
    ...
    func renderReplySurfaces() throws {
        ...
        let text =
            "[QA Topic](\(first)) [QA Work](\(second)) [Apple Materials](https://developer.apple.com/design/human-interface-guidelines/materials) [PDF](../paper.pdf)"
        ...
        for scheme in [ColorScheme.light, .dark] {
            let content = VStack(alignment: .leading, spacing: 20) {
                ...
                AgentChatMarkdown(
                    text:
                        "这是一条用于检查排版的最终回复。保留 **原文依据** 与解释的区别，支持中文和 English 的自然换行。\n\n- 原文依据\n- 解释与评价\n\n| 材料 | 作用 |\n|---|---|\n| 原文 | 核对引文 |\n| 笔记 | 保留讨论 |\n\n可以跨段连续选取。",
                    quoteSelection: { _ in })
                AgentChatReplyQuoteCard(
                    quote: .init(
                        conversationID: UUID(), messageID: "fixture",
    ...
    func renderTurnStates() throws {
        ...
        for (index, fixture) in [
            (ColorScheme.light, "zh-Hans", 300.0), (.dark, "zh-Hans", 420.0),
            (.light, "en", 420.0), (.dark, "en", 300.0),
        ].enumerated() {
            ...
            let content = VStack(alignment: .leading, spacing: 20) {
                ForEach(
                    Array(
                        [
                            AgentChatTurnPresentation.State.reading, .working, .responding,
                            .waitingForInput, .waitingForApproval, .completed, .interrupted, .failed, .uncertain,
    ...
    func renderNotePicker() async throws {
        ...
        let notes: [WorkspaceCatalogNote] = [
            ...
            .init(
                reference: .init(
                    vaultID: UUID(), vaultName: role.displayName, vaultRole: role,
    ...
    func renderLocalMaterials() throws {
        ...
        for failed in [false, true] {
            ...
            let material = AgentChatLocalMaterial(
                id: id, source: .file(URL(fileURLWithPath: "/Fixture/Research/原文与解释.pdf")),
                storedFileName: "\(id).pdf", fingerprint: .init(content: "synthetic"), kind: .pdf,
                pages: failed
                    ? []
                    : [
                        .init(number: 1, text: "Synthetic source passage.\n\nA distinction between the source and its interpretation."),
                        .init(number: 2, text: ""),
                    ], issue: failed ? .noText : nil)
            for scheme in [ColorScheme.light, .dark] {
    ...
    func renderPDFPageSelection() throws {
        ...
        for invalid in [false, true] {
            for scheme in [ColorScheme.light, .dark] {
                let content = AgentChatPDFPagesForm(
                    fileName: "原文与解释.pdf", pageCount: 12,
    ...
    func renderCapturedImageMaterial() throws {
        ...
        for origin in [AgentChatLocalMaterial.CaptureOrigin.clipboard, .drop] {
            let id = UUID()
            let material = AgentChatLocalMaterial(
                id: id, source: .imageCapture(origin), storedFileName: "\(id).png",
    ...
    func renderQuestionForm() throws {
        ...
        let questions: [AgentChatQuestion] = [
            .init(
                id: "approach", prompt: "我们从哪里展开这段讨论？",
                options: [
                    .init(label: "比较原文", description: "先核对两处原文，再讨论解释上的差异。"),
                    .init(label: "检查反对意见", description: "检验这一反对意见是否击中论证的前提。"),
        ...
        for state in ["empty", "custom", "pending", "tool"] {
            for scheme in [ColorScheme.light, .dark] {
                ...
                let content = AgentChatQuestionForm(
                    questions: questions, answers: .constant(answers),
    ...
    func renderUpdateComparison() async throws {
        ...
        let preview = AgentNoteUpdatePreview(
            noteID: UUID(), relativePath: "Topics/原文与解释.md",
    ...
    func renderDelegation() throws {
        ...
        let report = AgentChatDelegation(
            operation: .spawnAgent, senderThreadID: "fixture-parent",
            prompt: "核对所提供的两处原文，保留页码与不确定之处。",
            targets: [
                .init(id: "fixture-child-a", path: "/root/source-check", state: .running),
                .init(
                    id: "fixture-child-b", path: "/root/objection-check", state: .completed,
    ...
    func renderRuntimeApprovals() throws {
        ...
        let output = repository.appendingPathComponent(".build/agent-chat-evolution/renders")
        try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)
        let command = try CodexChatRuntimeApproval.parse(
            method: "item/commandExecution/requestApproval",
            params: [
                "command": .string("python3 inspect_sources.py --only supplied"), "cwd": .string("/Fixture/Research"),
                "reason": .string("读取所选论文中的参考文献。"), "availableDecisions": .array([.string("accept"), .string("acceptForSession"), .string("decline")]),
            ], item: nil)
        let permissions = try CodexChatRuntimeApproval.parse(
            method: "item/permissions/requestApproval",
            params: [
                "cwd": .string("/Fixture/Research"),
                "permissions": .object([
                    ...
                    "fileSystem": .object([
                        "entries": .array([
                            ...
                            .object([
                                "access": .string("write"), "path": .object(["type": .string("glob_pattern"), "pattern": .string("/Fixture/Drafts/**/*.md")]),
                            ]),
                            .object(["access": .string("deny"), "path": .object(["type": .string("path"), "path": .string("/Fixture/Private")])]),
                        ])
                    ]),
                ]),
            ], item: nil)
        let network = try CodexChatRuntimeApproval.parse(
            method: "item/commandExecution/requestApproval",
            params: [
                "command": .string("opaque network transport"),
                "networkApprovalContext": .object([
                    "host": .string("sources.example.test:8443"), "protocol": .string("https"),
                ]),
            ], item: nil)
        let files = try CodexChatRuntimeApproval.parse(
            ...
            item: .init(
                .object([
                    ...
                    "changes": .array([
                        .object([
                            "path": .string("/Fixture/原文与解释.md"), "kind": .object(["type": .string("update"), "move_path": .string("/Fixture/修订稿.md")]),
                            "diff": .string("-旧的解释\n+区分原文与我们的重构\n"),
                        ])
                    ]),
                ])))
        for (name, request, decision) in [
            ("command", command, Optional<AgentChatRuntimeApproval.Decision>.none),
            ("permissions", permissions, nil), ("files", files, nil), ("network", network, nil), ("submitted", permissions, .session),
        ] {
            for scheme in [ColorScheme.light, .dark] {
                let content = AgentChatRuntimeApprovalView(
    ...
    func renderMethodSettings() async throws {
        ...
        for kind in AgentChatToolConnection.Kind.allCases {
            ...
            edit.connection = .init(
                name: "research-catalog-\(kind.rawValue)", kind: kind,
    ...
    func renderFindControls() throws {
        ...
        try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)
        for scheme in [ColorScheme.light, .dark] {
            let content = AgentChatFindBar(
                query: .constant("情绪与理由"), focusRequest: nil,
