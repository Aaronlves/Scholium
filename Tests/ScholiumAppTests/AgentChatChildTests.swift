...
struct AgentChatChildTests {
  ...
  private func make(_ root: URL, prompt: String = "hold delegation") async throws -> (AgentChatController, AgentChatChildController) {
    ...
    parent.editDraft(prompt); parent.send()
  ...
  func reading() async throws {
    ...
    #expect(child.messages.contains { $0.value.activity?.kind == .read
    ...
    child.loadEarlier(); try await wait { !child.isWorking }
    ...
    unrelated.refresh(); try await wait { !unrelated.isWorking }
  ...
  func nestedReports() async throws {
    ...
    let owner = try #require(parent.selectedID), token = try #require(parent.token)
    ...
    nested.refresh(); try await wait { !nested.isWorking }
    ...
    nested.askParent(); try await wait { nested.receipt != nil }
  ...
  func interruption() async throws {
    ...
    child.refresh(); try await wait { !child.isWorking }
  ...
  func coordination() async throws {
    ...
    let material = AgentChatAttachment(noteID: UUID(), vaultID: UUID(), relativePath: "Unsent.md",
    ...
    parent.newConversation(); parent.editDraft("Visible unrelated draft")
    let visible = parent.selectedID
    ...
    child.askParent(); try await wait { child.receipt == .received && parent.state(for: owner) == .ready }
    ...
    child.cancel(); await parent.disconnect()
  ...
  func completedTargetDuringPreparation() async throws {
    ...
    child.editDraft("Keep this additional request"); child.askParent()
  ...
  func coordinationAdmission() async throws {
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    let (parent, child) = try await make(root)
    let owner = try #require(parent.selectedID), count = parent.selected?.messages.count
    ...
    parent.stop(); try await wait { parent.state == .ready }
    parent.setArchived(owner, archived: true)
    #expect(!child.canSend && !child.canEdit)
    child.askParent(); child.editDraft("Must not replace archived draft")
