...
extension MCPAppBridgeRequestRouterTests {
    ...
    func attachmentImageRelationship() async throws {
        ...
        let args = scope.merging(["attachment_id": MCPJSONValue.string(imported.record.id.uuidString), "expected_note_fingerprint": try #require(listing["note_fingerprint"]), "mode": .string("image")]) { _, new in new }
    ...
    func attachmentTextScope() async throws {
        ...
        func call(_ tool: ScholiumMCPToolName, _ extra: [String: MCPJSONValue] = [:]) async -> ScholiumMCPBridgeResponse {
            await router.handle(.init(tool: tool, arguments: ["triptych_id": .string(fixture.assignment.id.uuidString),
                "note_id": .string(fixture.topicNoteID.uuidString)].merging(extra) { _, new in new }))
        }
        ...
        #expect(try result(await call(.listAttachments, ["offset": .integer(1), "expected_listing_fingerprint": try #require(listing["listing_fingerprint"])]))["attachments"]?.arrayValue?.isEmpty == true)
        var read: [String: MCPJSONValue] = ["attachment_id": .string(attachment.record.id.uuidString),
        ...
        read["expected_fingerprint"] = first["fingerprint"]; read["max_utf8"] = .integer(65_536)
        ...
        #expect(await call(.listAttachments, ["offset": .integer(1), "expected_listing_fingerprint": try #require(listing["listing_fingerprint"])]).error?.code == .staleRevision)
        read["note_id"] = .string(fixture.analysisNoteID.uuidString)
        read["expected_note_fingerprint"] = .object(["sha256": .string(fixture.analysisFingerprint.sha256), "byte_count": .integer(fixture.analysisFingerprint.byteCount)])
        #expect(await call(.readAttachment, read).error?.code == .notFound)
        read["note_id"] = nil; read["expected_note_fingerprint"] = listing["note_fingerprint"]
        guard case .vaultRelative(let path) = attachment.record.location else { Issue.record("Expected copied attachment"); return }
    ...
    func attachmentPDFPage() async throws {
        ...
        for text in ["First page excluded", "Second page selected"] {
            ...
            let value = NSAttributedString(string: text, attributes: [NSAttributedString.Key(kCTFontAttributeName as String): CTFontCreateWithName("Helvetica" as CFString, 14, nil)])
            CTLineDraw(CTLineCreateWithAttributedString(value), context); context.endPDFPage()
        }
        context.closePDF(); try (data as Data).write(to: source)
        let attachment = try await handle.documents.attachDocument(at: source, to: target, management: .copyIntoTriptych)
        let router = MCPAppBridgeRequestRouter(runtime: fixture.runtime, flushEditors: { _ in }, openTriptychs: { [fixture.assignment] })
        var args: [String: MCPJSONValue] = ["triptych_id": .string(fixture.assignment.id.uuidString), "note_id": .string(fixture.topicNoteID.uuidString),
            "attachment_id": .string(attachment.record.id.uuidString), "expected_note_fingerprint": .object(["sha256": .string(note.fingerprint.sha256), "byte_count": .integer(note.fingerprint.byteCount)]), "mode": .string("text")]
