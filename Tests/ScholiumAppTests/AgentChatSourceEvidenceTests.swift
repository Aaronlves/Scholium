...
struct AgentChatSourceEvidenceTests {
    ...
    func zoteroScopeAndVersions() throws {
        ...
        let first = ZoteroReadReport(
            server: "scholium-zotero", tool: "zotero_read_original", reference: page,
            representation: .pdfText, fingerprint: String(repeating: "a", count: 64), range: .init(start: 0, end: 4, total: 40), excerpt: "Read")
        let second = ZoteroReadReport(
            server: "custom-zotero", tool: "zotero_read_original", reference: page,
        ...
        guard case .zotero(let reports) = context.evidence(for: page.url) else {
            Issue.record("Missing reports")
            return
        }
        #expect(reports == [first, second])
        let plain = ZoteroReadReport(
            server: "custom-zotero", tool: "zotero_read_original",
        ...
        var previous = event(first)
        previous.turnID = "previous"
        var failed = event(first)
        ...
        falseOrigin.sourceObservation = .zoteroReadReport(first)
        spoof.activity = falseOrigin
    ...
    func annotationIsSeparateMaterial() throws {
        ...
        let report = ZoteroReadReport(
            server: "custom-zotero", tool: "zotero_read_annotation", reference: reference,
        ...
        let corrupted = ZoteroReadReport(
            server: "custom-zotero", tool: "zotero_read_annotation", reference: reference,
    ...
    func read(_ start: Int, _ count: Int, end: Bool = false, revision other: DocumentFingerprint? = nil) -> AgentChatMessage {
        ...
        activity.sourceObservation = .noteRead(
            .init(
                noteID: noteID, fingerprint: other ?? revision,
    ...
    func rangesAndRevisions() throws {
        ...
        guard case .note(let coverage) = context.evidence(for: url) else {
            Issue.record("Missing current read observation")
            return
        }
        ...
        guard case .note(let all) = complete.evidence(for: url) else {
            Issue.record("Missing complete coverage")
            return
        }
        #expect(all.coversWholeSource && all.coversCitedLine)
        let changed = AgentChatReplySourceContext(
            reply: answer,
            history: [
                read(1, 10),
                read(
        ...
        guard case .note(let old) = changed.evidence(for: url) else {
            Issue.record("Missing matching revision")
            return
        }
        #expect(!old.coversWholeSource && old.ranges == [1...10])
        if case .differentRevision = context.evidence(
            for: AgentChatReference.url(
                noteID: noteID,
                revision: DocumentFingerprint(content: "Unobserved")))
        {
        } else {
            Issue.record("Version mismatch was concealed")
        }
    }
    ...
    func scopeAndOrigin() {
        let answer = reply()
        var previous = read(1, 5, end: true)
        previous.turnID = "previous"
        var failed = read(1, 5, end: true)
        failed.activity?.status = .failed
        ...
        var runtime = AgentChatMessage(role: .operation, text: "", activity: spoof)
        runtime.turnID = "turn"
    ...
    func suppliedMaterials() {
        ...
        var input = AgentChatMessage(role: .user, text: "Discuss this", localMaterials: [material])
        input.turnID = "turn"
        let answer = reply()
    ...
    func parsing() throws {
        ...
        guard case .noteRead(let observation) = AgentChatReadObservation.parse(value) else {
            Issue.record("Missing read evidence")
            return
        }
        ...
        #expect(
            try JSONDecoder().decode(
                AgentChatSourceObservation.self,
        ...
        value["start_line"] = .integer(Int.max)
        #expect(AgentChatReadObservation.parse(value) == nil)
        value["start_line"] = .integer(1)
        value["complete"] = .bool(false)
        #expect(AgentChatReadObservation.parse(value) == nil)
    }
}
