...
struct AgentChatCapabilityToolTests {
  ...
  func activeTurnCanConfigureCapabilities() async throws {
    ...
    let disabled = await controller.handle(.init(tool: .configureSkill, arguments: [
    ...
    let reenabled = await controller.handle(.init(tool: .configureSkill, arguments: [
    ...
    let addedRoot = await controller.handle(.init(tool: .configureSkill, arguments: [
    ...
    let added = await controller.handle(.init(tool: .configureTool, arguments: [
    ...
    #expect(addedConfiguration["connections"]?.arrayValue?.contains {
      $0.objectValue?["name"]?.stringValue == "agent-parser"
    } == true)

    let enabled = await controller.handle(.init(tool: .configureTool, arguments: [
    ...
    let settings = await controller.handle(.init(tool: .configureChat, arguments: [
