...
struct CodexChatToolConfigurationTests {
  private func snapshot(override: Bool = false) throws -> CodexChatToolConfiguration {
    let server: MCPJSONValue = .object(["url": .string("https://one.example/mcp"), "enabled": .bool(true),
  ...
  private func snapshot(server: MCPJSONValue, override: Bool = false) throws -> CodexChatToolConfiguration {
    ...
    return try .init(.object(["config": .object(["mcp_servers": .object(["fixture.other": .object(effective)])]),
      "layers": .array([.object(["name": .object(["type": .string("user"), "file": .string("/fixture/config.toml")]),
  ...
  func environmentReferences() throws {
    ...
    let remote = try self.snapshot(server: .object(["url": .string("https://one.example/mcp"),
    ...
    #expect(edits.contains { $0.objectValue?["keyPath"]?.stringValue?.hasSuffix(".bearer_token_env_var") == true
      && $0.objectValue?["value"] == .null })
    let local = try self.snapshot(server: .object(["command": .string("/fixture/server"), "env_vars": .array([
    ...
    let helper = try self.snapshot(server: .object(["url": .string("https://one.example/mcp"),
  ...
  func namesAndAddresses() throws {
    ...
    #expect(try snapshot.writeParameters(.init(name: "local", kind: .local, address: "/fixture/program ",
      arguments: ["literal $(value)", "two words"]), originalName: nil)["edits"]?.arrayValue?.first?.objectValue?["value"]?.objectValue?["command"]?.stringValue == "/fixture/program ")
  }
  @Test("Installed runtime applies version-checked tool configuration and removes a connection",
  ...
  func officialConfiguration() async throws {
    ...
    do {
      ...
      _ = try await runtime.request("initialize", params: ["clientInfo": .object(["name": .string("scholium-tool-config-test"), "version": .string("1")])])
      try await runtime.notify("initialized")
      let initial = try await runtime.request("config/read", params: ["includeLayers": .bool(true)])
      let layer = try #require(initial.objectValue?["layers"]?.arrayValue?.first {
      ...
      let params: [String: MCPJSONValue] = ["filePath": .string(file), "expectedVersion": .string(version),
        "reloadUserConfig": .bool(true), "edits": .array([.object([
      ...
      _ = try await runtime.request("config/batchWrite", params: ["filePath": .string(file), "expectedVersion": .string(nextVersion),
        "reloadUserConfig": .bool(true), "edits": .array([.object([
      ...
      let removedVersion = try #require(removed.objectValue?["layers"]?.arrayValue?.first {
        $0.objectValue?["name"]?.objectValue?["type"]?.stringValue == "user"
      }?.objectValue?["version"]?.stringValue)
      _ = try await runtime.request("config/batchWrite", params: ["filePath": .string(file), "expectedVersion": .string(removedVersion),
        "reloadUserConfig": .bool(true), "edits": .array([.object([
      ...
      let server = AgentChatToolConnection(name: "local-fixture", kind: .local, address: "/usr/bin/python3",
      ...
      #expect(connected)
      var disabled = server; disabled.enabled = false; disabled.environmentVariables = []
      ...
      let remote = AgentChatToolConnection(name: "remote-env-fixture", address: "https://example.invalid/mcp", enabled: false,
