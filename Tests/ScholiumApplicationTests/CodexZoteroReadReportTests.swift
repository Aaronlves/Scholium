...
struct CodexZoteroReadReportTests {
    ...
    func reportHistoryParity() throws {
        ...
        guard case .zoteroReadReport(let report) = activity.sourceObservation else { Issue.record("Missing report"); return }
        ...
        let event = try #require(try CodexChatTranscript.event(["method": .string("item/completed"),
        ...
        guard case .item(let live, _, _) = event.content else { Issue.record("Missing live item"); return }
        #expect(live == history.first?.items.first)
        guard case .activity(let restored) = live.content else { Issue.record("Missing restored report"); return }
    ...
    func invalidReports() {
        ...
        var pending = good; pending["status"] = .string("inProgress"); failures.append(pending)
        var failed = good; failed["error"] = .object(["message": .string("failed")]); failures.append(failed)
        var tool = good; tool["tool"] = .string("other_tool"); failures.append(tool)
        var absent = good; absent["result"] = .null; failures.append(absent)
        var args = good; var wrong = args["arguments"]!.objectValue!; wrong["library"] = .string("group:42"); args["arguments"] = .object(wrong); failures.append(args)
        for patch: [String: MCPJSONValue] in [
            ...
            var copy = good; var result = copy["result"]!.objectValue!; var value = result["structuredContent"]!.objectValue!
            value.merge(patch, uniquingKeysWith: { _, new in new }); result["structuredContent"] = .object(value); copy["result"] = .object(result)
        ...
        semanticFailure["result"] = .object(["content": .array([]), "structuredContent": .object([
    ...
    func imageReports() throws {
        ...
        CGImageDestinationAddImage(destination, image, nil); try #require(CGImageDestinationFinalize(destination))
        ...
        var result = item["result"]!.objectValue!; var value = result["structuredContent"]!.objectValue!
        ...
        result["structuredContent"] = .object(value); item["result"] = .object(result)
        let activity = try #require(CodexChatActivity.parse(item, completed: true))
        guard case .zoteroReadReport(let report) = activity.sourceObservation else { Issue.record("Missing image report"); return }
    ...
    func exactReportBytes() throws {
        ...
        guard case .zoteroReadReport(let report) = activity.sourceObservation else { Issue.record("Missing exact report"); return }
    ...
    private func original(text: String, server: String = "scholium-zotero") -> [String: MCPJSONValue] {
        ...
        let reference: MCPJSONValue = .object(["library": .object(["type": .string("user"), "id": .integer(0)]), "kind": .string("item"),
            "item_key": .string("ATTACH01"), "page": .null, "annotation_key": .null,
            "url": .string("zotero://select/library/items/ATTACH01")])
        let value: MCPJSONValue = .object(["source_kind": .string("zotero_original"), "original_file_read": .bool(true),
        ...
        return ["id": .string("call"), "type": .string("mcpToolCall"), "server": .string(server), "tool": .string("zotero_read_original"),
