...
struct CodexChatSourceObservationTests {
  ...
  func webActions() {
    var item: [String: MCPJSONValue] = ["id": .string("web"), "type": .string("webSearch"),
    ...
    guard case .webAccess(let access) = completed?.sourceObservation else { Issue.record("Page action lost"); return }
