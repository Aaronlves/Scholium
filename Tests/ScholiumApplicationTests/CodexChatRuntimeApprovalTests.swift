...
struct CodexChatRuntimeApprovalTests {
  ...
  func decisions() throws {
    var params: [String: MCPJSONValue] = ["kind": .string("writeStdin"), "command": .string("exact 中文 input\n"),
    ...
    params["availableDecisions"] = .array([.object(["acceptWithExecpolicyAmendment": .object(["execpolicy_amendment": .array([.string("echo")])])]), .string("decline")])
  ...
  func permissions() throws {
    let profile: MCPJSONValue = .object(["network": .object(["enabled": .bool(false)]), "fileSystem": .object([
      "read": .array([.string("/fixture/source")]), "entries": .array([
        .object(["access": .string("write"), "path": .object(["type": .string("glob_pattern"), "pattern": .string("/fixture/**/*.md")])]),
        .object(["access": .string("deny"), "path": .object(["type": .string("special"), "value": .object(["kind": .string("project_roots"), "subpath": .string("private")])])])]),
    ...
    #expect(request.presentation.permissions?.rules.count == 3 && request.presentation.permissions?.network == false && request.presentation.permissions?.globScanMaxDepth == 4)
    ...
    for unknown: MCPJSONValue in [.object(["allVolumes": .bool(true)]),
  ...
  func files() throws {
    let item: MCPJSONValue = .object(["type": .string("fileChange"), "changes": .array([
      .object(["path": .string("/fixture/old.md"), "diff": .string("-old\n+new\n"),
    ...
    let root = try CodexChatRuntimeApproval.parse(method: "item/fileChange/requestApproval",
