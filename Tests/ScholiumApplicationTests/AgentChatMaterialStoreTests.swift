...
struct AgentChatMaterialStoreTests {
  ...
  func clipboardImage() async throws {
    ...
    let context = try #require(CGContext(data: nil, width: 2, height: 3, bitsPerComponent: 8, bytesPerRow: 8,
  ...
  func failures() async throws {
    ...
    try handle.truncate(atOffset: 21 * 1_024 * 1_024); try handle.close()
