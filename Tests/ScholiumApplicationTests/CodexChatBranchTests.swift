...
struct CodexChatBranchTests {
  private func history(_ turns: [(String, String)]) -> MCPJSONValue {
    .object(["thread": .object(["id": .string("source"), "turns": .array(turns.map {
  ...
  func boundaries() throws {
    ...
    #expect(throws: CodexConnectionError.self) {
      try CodexChatBranch.turnIDs(in: history([("one", "completed"), ("one", "completed"), ("two", "completed")]),
  ...
  func projection() throws {
    ...
    question.turnID = "one"
    let receipt = UUID()
    var change = AgentChatMessage(role: .operation, text: "", changeID: receipt,
    ...
    let empty = try CodexChatBranch.project(source: source, threadID: "empty",
