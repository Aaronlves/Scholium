...
RUNTIME_FILES = ("SKILL.md", "routing-index.md")
FORBIDDEN_CHARS = set('<>:"|?*')
...
def build_zip(repo_root: Path, output: Path) -> None:
    ...
    if not distilled:
        raise RuntimeError("no distilled files found")
...
def main() -> int:
    ...
    expected_distilled_count = len(list((repo_root / "distilled").glob("*.md")))
    if summary["distilled_count"] != expected_distilled_count:
        raise RuntimeError(
            f"unexpected distilled count: {summary['distilled_count']} "
            f"(expected {expected_distilled_count})"
        )
