...
def crawl(
    ...
    existing = {path.stem for path in (SKILL_ROOT / "distilled").glob("*.md")}
        ...
        inventory.append(
            {
                ...
                "raw_file": str(raw_path.relative_to(SKILL_ROOT).as_posix()),
...
def main() -> int:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Reuse valid raw JSON already present in the selected output directory.",
    )
    ...
    if overrides_path is not None and not overrides_path.is_absolute():
        overrides_path = SKILL_ROOT / overrides_path
    return crawl(
        output_dir,
