...
def validate(distilled_dir: Path) -> int:
    ...
    if not distilled_dir.is_dir():
        errors.append(f"{distilled_dir}: distilled directory does not exist")
    ...
    if not records:
        errors.append(f"{distilled_dir}: no distilled Markdown files found")
