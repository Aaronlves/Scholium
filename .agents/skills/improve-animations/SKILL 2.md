...
For Scholium, survey SwiftUI, AppKit, TextKit, WebKit, and other reachable
motion surfaces as applicable. Use the CSS and browser examples only when the
audited code is actually web-based; do not require CSS properties or browser
media queries of native macOS code. Read `Docs/PRODUCT_GUIDE.md` and
`Docs/DESIGN_HANDBOOK.md`, route Apple claims through `apple-hig`, and pair
with `scholium-apple-design` and `scholium-performance-audit` for the native
product interpretation and measured runtime concerns.
