...
When reviewing Scholium, inspect native SwiftUI/AppKit behavior as well as
reachable WebEditor or WebKit motion. Translate the web-specific standards into
the platform instead of flagging native code for lacking CSS mechanisms. Use
`Docs/PRODUCT_GUIDE.md` and `Docs/DESIGN_HANDBOOK.md` for product and interface
meaning, `apple-hig` for Apple guidance, and `scholium-apple-design` for the
Scholium interpretation. Cite the actual framework boundary and leave API
availability claims to the selected SDK and compiler.
