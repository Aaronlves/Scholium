...
| “候选 Swift index 已通过 shadow 证据；现在切换 production backend 并删除旧 route。” | Select Swift index-engine Cutover together with repository engineering Architecture Cutover; move every consumer and delete the old backend without a compatibility route or dual writer. |
...
| “只诊断 direct Agent Run 在重启和重新配对后的 Session、Context、Result 与 recovery 是否一致。” | Select Research Action collaboration for the functional lifecycle and add trust only for the credential or authorization boundary; remain read-only. |
| “修复 Result 在 outcome-unknown 写入收敛前形成 Record 的问题。” | Select Research Action collaboration for the Run-to-Record lifecycle; add repository engineering only if live ownership proves an actual cross-layer implementation. |
| “只修 Actions sheet 的焦点和最小宽度，不改变 Run 或 Agent 合同。” | Select native-interface implementation plus UI verification; do not load Research Action collaboration for presentation-only work. |
| “修改 bundled Analyze Method 的哲学分析方法，不改 Agent 协议。” | Stop at the release-shipped academic-method boundary; do not use the developer Research Action skill or toolkit maintenance to edit philosophical content. |
...
| “实现已批准的检查器方案，并让我验收。” | Select interface implementation, then use UI verification only for its distinct interaction or human-acceptance output. Keep visual approval, realistic task use, accessibility, and final verification separate. |
...
| “我只改了一个局部 UI 按钮，帮我验证。” | Run the owning deterministic tests and select UI-verification automated mode only for the affected interaction boundary. Reuse one representative journey and add an adjacent state only for a concrete failure mode; do not run or claim a complete UI suite unless the current repository authority identifies an explicit gate. |
| “替我验收 VoiceOver；我不懂自动化。” | Select UI-verification human-acceptance mode, automate safe setup and postconditions, then give one minimal genuine VoiceOver action and observable question. Do not claim synthetic events are human acceptance. |
