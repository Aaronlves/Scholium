...
Use the repository's approved snapshot tool when pixel-level regression testing
is justified. Adding or changing a dependency requires the project's normal
dependency review. Snapshot tests do not replace interaction or accessibility
tests.
...
class ProfileViewSnapshotTests: XCTestCase {
    func testProfileView() {
        ...
        // A project-specific scalable typography configuration
        assertSnapshot(of: view.environment(\.readerTextScale, 1.4),
                       as: .image, named: "largeText")
    }
}
```

Choose variants for the actual platform. For Scholium on macOS, cover light and
dark appearance, Increased Contrast, and the app's scalable reader typography
where those states affect the reviewed view. Do not import an iOS-only Dynamic
Type matrix into a macOS test plan.
...
8. **Unclear XCTest migration boundaries.** Apple allows XCTest and Swift
   Testing in one target during migration, but framework APIs should not be
   mixed in one test. Prefer separate files when it keeps imports and runner
...
- [ ] New Swift unit and integration tests use Swift Testing when compatible;
      UI and performance tests remain on XCTest/XCUITest
