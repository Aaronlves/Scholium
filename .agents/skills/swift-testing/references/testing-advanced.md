...
Use warnings for:
- Exploratory performance observations that are not acceptance gates; a real
  performance threshold must still fail its dedicated benchmark
...
@Test func renderedOutput() async throws {
    let image: NSImage = try renderPreviewFixture()
    Attachment.record(image, named: "preview-output", as: .png)
}
```
...
Do not use exit tests on unsupported runtime targets such as iOS, tvOS, or
watchOS. Move fatal behavior behind a smaller pure Swift API, test non-exiting
branches directly, and reserve subprocess exit tests for a supported host or
tool target.
