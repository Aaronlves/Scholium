...
def list_signposts(
    ...
    Reads two complementary schemas:
      ...
      * `os-signpost`: raw rows used here only for explicit point events. Most
    ...
    # also shows up as a row in `os-signpost-interval`. Use only the current
    # pre-paired schema for intervals and mine `os-signpost` for point events.
    ...
    if OS_SIGNPOST_SCHEMA in toc_schemas:
        more_events = _read_event_schema(trace_path, run=run)
...
def _read_event_schema(
    ...
    """Read explicit point events from the os-signpost schema."""
    ...
    for row in stream:
        ...
        if etype in ("begin", "interval begin", "start", "end", "interval end", "stop"):
            continue
