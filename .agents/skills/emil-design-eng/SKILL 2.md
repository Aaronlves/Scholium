...
## Scholium integration boundary

Apply this craft lens to Scholium only through its native macOS and research
authorities. `Docs/PRODUCT_GUIDE.md` owns product behavior;
`Docs/DESIGN_HANDBOOK.md` owns stable interface decisions; and `apple-hig`
owns Apple platform guidance. The CSS, React, browser, and Motion examples in
this skill are illustrative and must not be treated as native API guidance or
as Scholium defaults. Pair with `scholium-apple-design` for the platform and
product interpretation, and with the relevant implementation or performance
