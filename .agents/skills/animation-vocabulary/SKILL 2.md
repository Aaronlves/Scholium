...
This is a cross-platform vocabulary aid, not a SwiftUI/AppKit API reference or
a Scholium product contract. Translate web-oriented examples into the native
platform when working on Scholium. Use `Docs/PRODUCT_GUIDE.md` and
`Docs/DESIGN_HANDBOOK.md` for product meaning, and route Apple platform claims
to `apple-hig`.
