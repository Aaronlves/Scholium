...
Before using async `defer` (SE-0493) or cancellation shields (SE-0504), verify
that the resolved compiler accepts the syntax and that the deployment target
satisfies the exact SDK availability of `withTaskCancellationShield`. Establish
those facts from the live toolchain and official documentation instead of a
remembered Xcode generation or an iOS-only availability statement.
