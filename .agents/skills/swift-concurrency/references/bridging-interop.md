...
Use `withCheckedContinuation` (non-throwing) or
`withCheckedThrowingContinuation` (throwing) to bridge one-shot
completion-handler APIs into async/await. Verify compiler and deployment
availability in the active project rather than copying an iOS-only range into a
macOS target.
...
For new code, prefer the `AsyncStream.makeStream(of:)` or
`AsyncThrowingStream.makeStream(of:throwing:)` factory when the selected
toolchain provides it; the returned stream and continuation make ownership
explicit. Regardless of initializer, choose a bounded buffering policy for
high-rate producers and finish the continuation from teardown or
`onTermination`. Verify the exact overload against the selected SDK.
...
| `DispatchQueue.global().async { }` | An owned task calling an `@concurrent` helper for CPU work, when supported; `Task {}` alone may inherit actor context |
| `DispatchGroup` | `async let` or `TaskGroup` |
| `DispatchSemaphore` | Redesign as actor isolation, an async stream, or another structured async coordination primitive; do not mechanically replace it |
