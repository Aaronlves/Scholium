...
- Continuous long-source analysis is now the official workflow: one source-level Analysis is updated as bounded three-pass units accumulate. The current Research Status editor records scope and limitations, and the bundled Source Analysis method governs cumulative updates. A dedicated long-source progress presentation remains future work.
...
- **Open Scholia…** is the single prominent document-local doorway. A Dialogue accelerator opens that panel with Dialogue selected instead of creating a second standalone route. The shared panel follows the prototype's centered research-sheet composition: note identity and dismissal flank a centered role-aware segmented control, while dense scholarly content remains on a restrained 660-point reading measure. It expands to 940 points in wide windows and preserves 36-point outer margins in compact windows.
...
1. Complete manual VoiceOver, Full Keyboard Access, visual 200% text, appearance, contrast, reduced-transparency, and reduced-motion acceptance. The clean-account XCUITest already drives the three vault Open panels plus the containing-folder authorization panel, verifies the portable manifest, relaunches with the same isolated home, and renames the Triptych without a `manifest.json` permission error. The focused Critique XCUITest verifies agent attribution, deterministic Specific Findings disclosure, and exact Source navigation on a disposable Triptych.
