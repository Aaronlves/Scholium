...
public actor AgentChatMaterialStore {
    ...
    public func stage(_ source: URL) throws -> AgentChatLocalMaterial {
        ...
        let kind: AgentChatLocalMaterial.Kind =
            type == .pdf
            ? .pdf
            : (type?.conforms(to: .image) == true
                ? .image : (type?.conforms(to: .text) == true || source.pathExtension.lowercased() == "md" ? .text : .unsupported))
        var data: Data?
        ...
        do {
            ...
            coordinator.coordinate(readingItemAt: source, options: .withoutChanges, error: &coordinationError) { location in
                do {
                    ...
                    guard (before.fileSize ?? Int.max) <= Self.maximumFileBytes else {
                        failure = .tooLarge
                        return
                    }
        ...
        switch kind {
        case .text:
            if data.count > Self.maximumTextBytes {
                issue = .tooLarge
            } else if let decoded = String(validating: data, as: UTF8.self) {
                text = decoded
            } else {
                issue = .unreadable
            }
            ...
            if let pdf = PDFDocument(data: data) {
                if pdf.isLocked {
                    ...
                    for index in 0..<pdf.pageCount {
                        ...
                        if total > Self.maximumTextBytes {
                            issue = .tooLarge
                            pages = []
                            break
                        }
        ...
        return .init(
            id: id, source: .file(source), storedFileName: name, fingerprint: .init(data: data),
    ...
    public func stageImageCapture(_ data: Data, origin: AgentChatLocalMaterial.CaptureOrigin) throws -> AgentChatLocalMaterial {
        ...
        do {
            ...
            return .init(
                id: id, source: .imageCapture(origin), storedFileName: name, fingerprint: .init(data: png as Data), kind: .image,
    ...
    private static func imageIssue(_ data: Data, allowsTIFF: Bool = false) -> AgentChatLocalMaterial.Issue? {
        guard let image = CGImageSourceCreateWithData(data as CFData, nil), CGImageSourceGetCount(image) == 1,
            ...
            [UTType.png, .jpeg, .webP, .gif].contains(actual) || (allowsTIFF && actual == .tiff)
    ...
    public func validatedURL(for material: AgentChatLocalMaterial) throws -> URL {
        ...
        } else if material.capturedFileName != nil || material.capturedFingerprint != nil {
    ...
    private func readSnapshot(name: String, id: UUID, fingerprint: DocumentFingerprint) throws -> (url: URL, data: Data) {
        ...
        guard attributes.isRegularFile == true, attributes.isSymbolicLink != true,
            (attributes.fileSize ?? Int.max) <= Self.maximumFileBytes
        else { throw CocoaError(.fileReadCorruptFile) }
    ...
    public func renderPDFPages(_ selection: String, from material: AgentChatLocalMaterial) throws -> AgentChatLocalMaterial {
        ...
        do {
            ...
            try Task.checkCancellation()
            return .init(
                id: id, source: material.source, storedFileName: sourceName,
    ...
    public func validatedPageImageURLs(for material: AgentChatLocalMaterial) throws -> [URL] {
        ...
        guard material.kind == .pdf, material.pageImages.count <= AgentChatPDFPageSelection.maximumPages,
            Set(material.pageImages.map(\.number)).count == material.pageImages.count
        else { throw CocoaError(.fileReadCorruptFile) }
    ...
    public func discard(_ material: AgentChatLocalMaterial) throws {
        ...
        if let captured = material.capturedFileName {
            guard captured == URL(fileURLWithPath: captured).lastPathComponent, captured.hasPrefix(material.id.uuidString + ".") else {
                throw CocoaError(.fileWriteInvalidFileName)
            }
            names.append(captured)
        }
        for page in material.pageImages {
            guard page.number > 0, page.storedFileName == material.id.uuidString + ".page-\(page.number).png" else {
                throw CocoaError(.fileWriteInvalidFileName)
            }
