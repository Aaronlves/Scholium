...
public enum CodexChatBranch {
  ...
  public static func project(source: AgentChatConversation, threadID: String,
    retaining turnIDs: [String], boundary: String,
    position: AgentChatBranchOrigin.Position) throws -> AgentChatConversation {
  ...
  public static func retainedMessages(source: AgentChatConversation, through turnIDs: [String]) throws -> [AgentChatMessage] {
    guard !turnIDs.contains(where: \.isEmpty), Set(turnIDs).count == turnIDs.count,
      ...
      source.messages.allSatisfy({ $0.turnID?.isEmpty == false }) else { throw CodexConnectionError.invalidMessage }
  ...
  public static func confirm(_ result: MCPJSONValue, threadID: String,
    expected turnIDs: [String]) throws {
    let turns = try CodexChatTranscript.history(result, threadID: threadID)
