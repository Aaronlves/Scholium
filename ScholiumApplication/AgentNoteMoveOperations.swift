...
extension WorkspaceHandle {
    ...
    func captureAgentMove(
        plan: IncomingLinkRewritePlan, noteID: UUID, expectedFingerprint: DocumentFingerprint,
        expectedPlanFingerprint: DocumentFingerprint
    ) async throws -> (preview: AgentNoteMovePreview, beforeData: Data, afterData: Data, move: AgentMoveEvidence) {
        ...
        for effect in preview.effects {
            ...
            if effect.noteID == noteID {
                primaryBefore = current.sourceBytes
                primaryAfter = after
            } else {
                linked.append(.init(effect: effect, beforeData: current.sourceBytes, afterData: after))
            }
        }
    ...
    func prepareAgentMoveEvidence(
        plan: IncomingLinkRewritePlan, noteID: UUID, expectedFingerprint: DocumentFingerprint,
        authorization: AgentMoveAuthorization
    ) async throws {
        let captured = try await captureAgentMove(
            plan: plan, noteID: noteID, expectedFingerprint: expectedFingerprint,
            expectedPlanFingerprint: authorization.planFingerprint)
        try Task.checkCancellation()
        _ = try await services.agentChangeStore.prepare(
            id: authorization.changeID, operation: .move, noteID: noteID,
    ...
    func confirmAgentMoveEvidence(id: UUID, commit: TriptychMoveCommit) async throws {
        do {
            ...
            guard let move = evidence.move, move.primary.source == commit.movedNote, move.primary.destination == commit.destination,
                move.primary.beforeFingerprint == commit.previousRevision
            ...
            for linked in move.linkedSources {
                guard let rewrite = commit.rewrites.first(where: { $0.note == linked.effect.destination }),
                    ...
                    rewrite.rewrittenOccurrences == linked.effect.rewrittenOccurrences
                else { throw AgentChangeError.mismatchedBinding(id) }
    ...
    func agentMoveComparison(preview: AgentNoteMovePreview, before: Data, after: Data, move: AgentMoveEvidence, reversed: Bool = false) throws
        -> AgentNoteUpdatePreview
    {
        func comparison(_ before: Data, _ after: Data) throws -> ExactSourceComparison {
            let start = reversed ? after : before
            let end = reversed ? before : after
            return try ExactSourceComparisonBuilder.build(
                startingData: start, endingData: end,
                startingRevision: DocumentFingerprint(data: start), endingRevision: DocumentFingerprint(data: end))
        }
        return try .init(
            noteID: preview.noteID, relativePath: preview.source.relativePath, comparison: comparison(before, after), movePreview: preview,
    ...
    func previewAgentNoteMove(noteID: UUID, expectedFingerprint: DocumentFingerprint, to relativePath: String) async throws -> AgentNoteMovePreview {
        ...
        let preflight = IncomingLinkRewritePlan(
            movedNote: plan.movedNote, destination: plan.destination,
    ...
    func agentMovePreview(plan: IncomingLinkRewritePlan, noteID: UUID, expectedFingerprint: DocumentFingerprint) async throws -> AgentNoteMovePreview {
        ...
        for source in sourceIDs.sorted() {
            ...
            effects.append(
                .init(
                    noteID: identity.id, role: role, source: source,
        ...
        return try .init(
            noteID: noteID, role: try vault(id: plan.movedNote.vaultID).role, source: plan.movedNote,
