...
public struct CodexChatToolConfiguration: Sendable {
    ...
    public init(_ result: MCPJSONValue, home: URL) throws {
        ...
        connections = try servers.keys.sorted().map { name in
            ...
            return .init(
                name: name, kind: local ? .local : .remote, address: address,
    ...
    public func requiresAccessConfirmation(_ connection: AgentChatToolConnection, originalName: String?) -> Bool {
        guard let name = originalName, let old = connections.first(where: { $0.name == name }),
            let raw = rawConnections[name]?.objectValue
        else { return false }
    ...
    public func writeParameters(
        _ connection: AgentChatToolConnection, originalName: String?, removing: Bool = false,
        reuseAccessSettings: Bool = false
        ...
        return [
            "filePath": .string(file), "expectedVersion": .string(version), "reloadUserConfig": .bool(true),
            "edits": .array(edits),
        ]
    }
...
extension CodexAppServer {
    ...
    public func writeChatTool(
        _ connection: AgentChatToolConnection, originalName: String?,
        snapshot: CodexChatToolConfiguration, removing: Bool = false, reuseAccessSettings: Bool = false
    ) async throws -> Bool {
        let result = try await request(
            "config/batchWrite",
            params: snapshot.writeParameters(
        ...
        guard let reply = result.objectValue, reply["filePath"]?.stringValue == snapshot.file,
            ...
            ["ok", "okOverridden"].contains(status)
        else { throw CodexConnectionError.invalidMessage }
