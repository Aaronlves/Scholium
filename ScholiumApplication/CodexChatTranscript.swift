...
public struct CodexChatOperationContext: Sendable {
    ...
    public func toolQuestion() throws -> (identity: String, arguments: MCPJSONValue?)? {
        ...
        guard let server = item["server"]?.stringValue, !server.isEmpty,
            let tool = item["tool"]?.stringValue, !tool.isEmpty
...
public enum CodexChatTranscript {
    ...
    public static func history(
        _ value: MCPJSONValue, threadID: String,
        delegationOrigins: [String: String] = [:]
    ) throws -> [AgentChatTranscript.Turn] {
        guard let thread = value.objectValue?["thread"]?.objectValue,
            thread["id"]?.stringValue == threadID,
            let values = thread["turns"]?.arrayValue
        else { throw CodexConnectionError.invalidMessage }
    ...
    public static func turns(
        _ values: [MCPJSONValue], threadID: String,
        delegationOrigins: [String: String] = [:]
    ) throws -> [AgentChatTranscript.Turn] {
        var seen: Set<String> = []
    ...
    public static func turn(
        _ value: MCPJSONValue?, threadID: String,
        delegationOrigins: [String: String] = [:], requiresFullItems: Bool = false
    ) throws -> AgentChatTranscript.Turn {
        ...
        guard ["full", "summary", "notLoaded"].contains(coverage),
            !requiresFullItems || coverage == "full"
        else { throw CodexConnectionError.invalidMessage }
        ...
        let started = timestamp("startedAt")
        let completed = timestamp("completedAt")
        let duration = object["durationMs"]?.intValue
        let ordered = started == nil || completed == nil || completed! >= started!
        let timing = AgentChatTurnTiming(
            startedAt: ordered ? started : nil,
        ...
        let items = try values.compactMap { value -> AgentChatTranscript.Item? in
            ...
            return try item(
                object, completed: object["status"]?.stringValue != "inProgress",
    ...
    private static func item(
        _ object: [String: MCPJSONValue], completed: Bool, threadID: String,
        turnStatus: AgentChatTranscript.Turn.Status? = nil
    ) throws -> AgentChatTranscript.Item? {
        let id = try identifier(object["id"])
        ...
        if type == "userMessage" {
            ...
            return .init(
                id: id, content: .user(text: text.joined(separator: "\n\n"), hasAdditionalMaterial: additional),
                clientMessageID: try optionalIdentifier(object["clientId"]))
        }
        guard
            var activity = CodexChatActivity.parse(
                object, completed: completed, threadID: threadID,
                includesManagedTools: true)
        else { return nil }
        ...
        let managed =
            type == "mcpToolCall" && object["server"]?.stringValue == "scholium"
