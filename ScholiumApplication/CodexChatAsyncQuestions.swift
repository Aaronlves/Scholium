...
public enum CodexChatAsyncQuestions {
    public static func parse(_ object: [String: MCPJSONValue], id: String, text: String) throws -> [AgentChatQuestion] {
        ...
        if object["questions"] == nil || object["questions"] == .null {
            values = []
        } else if let array = object["questions"]?.arrayValue, array.count <= 16 {
            values = array
        } else {
        ...
        return try values.enumerated().map { index, value in
            ...
            if question["options"] == nil || question["options"] == .null {
                options = []
            } else if let array = question["options"]?.arrayValue, array.count <= 64 {
                options = array
            } else {
                throw CodexConnectionError.invalidMessage
            }
