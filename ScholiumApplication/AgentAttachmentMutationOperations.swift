...
extension WorkspaceHandle {
    func attachmentPlan(noteID: UUID, expectedSource: DocumentFingerprint, update: AgentAttachmentUpdate) async throws -> AgentRecordPlan {
        ...
        guard fingerprint == update.listingFingerprint else {
            throw AgentCollaborationError.staleRevision(expected: update.listingFingerprint, current: fingerprint)
        }
        let records = try await services.controlStore.documentAttachmentRecords()
        let old = records.first { $0.id == update.attachmentID }
        switch update.action {
        case .add:
            guard try await !services.controlStore.attachmentRecords().contains(where: { $0.id == update.attachmentID }), old == nil,
                !listing.attachments.contains(where: { $0.id == update.attachmentID })
        ...
        if update.action != .remove {
            guard let source = update.source else {
                throw AgentCollaborationError.invalidRequest("Select one existing document attachment as the material source.")
            }
            ...
            guard sourceFingerprint == source.listingFingerprint else {
                throw AgentCollaborationError.staleRevision(expected: source.listingFingerprint, current: sourceFingerprint)
            }
            guard let original = records.first(where: { $0.id == source.attachmentID && $0.noteID == source.noteID }),
                sourceListing.attachments.contains(where: {
                    $0.id == original.id && $0.available && $0.relationship == .document && $0.location == original.location
                })
            else {
            ...
            let bytes = try await agentDocumentBytes(
                noteID: source.noteID, target: sourceNote.id,
            ...
            guard try currentListing.fingerprint(triptychID: id, noteID: source.noteID) == sourceFingerprint else {
        ...
        } else if update.source != nil {
            throw AgentCollaborationError.invalidRequest("Removing a relationship accepts no replacement source.")
        }
        let plan = try AgentRecordPlan(
            target: target, noteID: noteID, operation: .attachment,
            before: AgentRecordChange.attachment(old), after: AgentRecordChange.attachment(new), metadataRevision: nil, copy: copy,
            attachmentListingFingerprint: update.listingFingerprint, sourceListing: sourceProof)
        _ = try plan.preview()
        return plan
    }
    func agentDocumentBytes(noteID: UUID, target: VaultQualifiedNoteID, attachmentID: UUID, expected: DocumentFingerprint) async throws -> Data {
        let lease = try await prepareDocumentAttachmentPreview(
        ...
        do {
            ...
            guard DocumentFingerprint(data: bytes) == expected else {
                throw AgentCollaborationError.staleRevision(expected: expected, current: .init(data: bytes))
            }
