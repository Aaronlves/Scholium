# Conditional Agent Transport

Read this reference only for an unprepared direct-agent task that must discover or retrieve a Skill package, for cold CLI establishment, or when a prepared request lacks the typed action required for recovery. Prepared Research Functions already carry their exact task state, package revisions, permissions, and typed continuation actions; do not load this generic transport contract merely because the CLI is available.
...
Outside a prepared Research Function, classify the intellectual operation and retrieve exactly one complete matching method only when interpretation, analysis, development, assessment, or revision requires it:
...
## Retrieve declared conditional resources
...
Within a method-unresolved Research Function preflight, select conditional Workflow references only through the preparation's typed `select_resources` action. Generic Skill retrieval is not attached to the run and must not be reported as loaded-run evidence.

## Execute typed actions safely

For prepared runs, prefer the typed `nextActions` argument vectors returned by Scholium. Execute the arguments without shell interpolation and replace every required marker in an input template with checked evidence before submission. Use `function show` for process-loss recovery. Never treat successful transport, persistence, or status output as evidence that the philosophical method was adequate.
