...
public struct AgentChatLocalMaterial: Codable, Equatable, Identifiable, Sendable {
  ...
  public struct PageImage: Codable, Equatable, Sendable {
    ...
    public init(number: Int, storedFileName: String, fingerprint: DocumentFingerprint) {
      self.number = number; self.storedFileName = storedFileName; self.fingerprint = fingerprint
    }
  }
  public let id: UUID
  ...
  public init(id: UUID, source: Source, storedFileName: String?, fingerprint: DocumentFingerprint?,
    ...
    self.id = id; self.source = source; self.storedFileName = storedFileName; self.fingerprint = fingerprint
    self.kind = kind; self.text = text; self.pages = pages; self.pageImages = pageImages; self.issue = issue
    self.capturedFileName = capturedFileName; self.capturedFingerprint = capturedFingerprint
  }
}
