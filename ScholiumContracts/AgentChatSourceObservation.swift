...
public enum AgentChatSourceObservation: Codable, Equatable, Sendable {
  ...
  public struct NoteRead: Codable, Equatable, Sendable {
    ...
    public init(noteID: UUID, fingerprint: DocumentFingerprint, startLine: Int,
      lineCount: Int, reachedEnd: Bool, excerpt: String, excerptIsTruncated: Bool) {
      self.noteID = noteID; self.fingerprint = fingerprint; self.startLine = startLine
      self.lineCount = lineCount; self.reachedEnd = reachedEnd
      self.excerpt = excerpt; self.excerptIsTruncated = excerptIsTruncated
    }
  }
  ...
  public struct WebAccess: Codable, Equatable, Sendable {
    ...
    public let url: URL
    public init(action: Action, url: URL) { self.action = action; self.url = url }
  }
}
