...
public struct AgentNoteDisplayTarget: Sendable {
    ...
    public init(
        triptychID: UUID, noteID: UUID, note: VaultQualifiedNoteID, fingerprint: DocumentFingerprint,
        source: Data, startUTF8: Int? = nil, endUTF8: Int? = nil, expectedText: String? = nil
    ) throws {
        func invalid() -> ScholiumMCPFailure {
            .init(
                code: .invalidRequest, message: "The Note display range is incomplete or does not match exact current source.",
                recovery: "Read the Note again and use its full-source UTF-8 offsets and exact text.")
        }
        ...
        if startUTF8 == nil && endUTF8 == nil && expectedText == nil {
            range = nil
            excerpt = nil
            return
        }
        guard let lower = startUTF8, let upper = endUTF8, let expectedText,
            ...
            Range(NSRange(location: prefix.utf16.count, length: ending.utf16.count - prefix.utf16.count), in: text) != nil
        else { throw invalid() }
        func position(_ prefix: String) -> (Int, Int) {
            ...
            for character in prefix {
                if character == "\n" || character == "\r" || character == "\r\n" {
                    ...
                    column += String(character).utf16.count
                }
            }
            return (line, column)
        }
        let start = position(prefix)
        let end = position(ending)
        range = .init(
            utf16LowerBound: prefix.utf16.count, utf16UpperBound: ending.utf16.count,
