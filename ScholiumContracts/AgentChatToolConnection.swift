...
public struct AgentChatToolConnection: Equatable, Identifiable, Sendable {
  ...
  public init(name: String = "", kind: Kind = .remote, address: String = "", arguments: [String] = [],
    ...
    self.name = name; self.kind = kind; self.address = address; self.arguments = arguments
    self.enabled = enabled; self.isEditable = isEditable
    self.bearerTokenVariable = bearerTokenVariable; self.environmentVariables = environmentVariables
