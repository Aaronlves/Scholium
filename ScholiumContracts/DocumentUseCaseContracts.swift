...
/// Stable delivery encoding for a Property value. Public CLI and Agent JSON
    ...
    init(from decoder: Decoder) throws {
        ...
        if container.decodeNil() {
            ...
            throw DecodingError.dataCorruptedError(
                in: container,
                debugDescription: "Property values must be ordinary JSON values."
            )
        }
    }
...
public struct AnalysisCreationMetadata: Codable, Hashable, Sendable {
    ...
    public init(
        ...
        guard properties.count <= PropertyContractCatalog.analysisCanonicalKeys.count,
              Set(properties.map(\.key)).count == properties.count,
              !properties.contains(where: { $0.key == "type" }) else {
            throw DocumentCreationError.invalidMetadata([])
        }
        ...
        self.properties = properties.sorted { lhs, rhs in
...
public enum DocumentCreationError: LocalizedError, Equatable, Sendable {
    ...
    case analysisSeedCollision(String)
    ...
    public var errorDescription: String? {
        ...
        case .invalidPropertyKey(let key):
            return "The managed creation property key is invalid: \(key)"
        case .invalidBody:
        ...
        case .analysisSeedCollision(let key):
            return "The field \(key) is already supplied by the Analysis New Note YAML seed."
        case .settingsRevisionChanged:
            return "The Properties settings changed after this creation was authorized. Reload the current Run before creating the note."
        case .portableIdentityAlreadyExists:
