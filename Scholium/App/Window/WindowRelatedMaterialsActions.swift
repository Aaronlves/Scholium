...
extension WindowModel {
    ...
    func findRelatedMaterials(automatic: Bool = false) {
        ...
        guard let capabilities = windowWorkspaceController.activeCapabilities,
            ...
            let stableID = note.workspaceSnapshot?.stableIdentity.resolvedID
        else {
        ...
        materials.find(
            capture: { [weak self] in
                ...
                do { selection = try await editor.selectedSourceSnapshot() } catch { throw RelatedMaterialsError.selectionRequired }
                guard let self,
    ...
    func refreshRelatedMaterials() {
        guard let capabilities = windowWorkspaceController.activeCapabilities,
            let seed = researchController.relatedMaterials.seed
        else { return }
