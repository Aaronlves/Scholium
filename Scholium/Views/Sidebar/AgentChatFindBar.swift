...
struct AgentChatFindBar: View {
  ...
  var body: some View {
    VStack(spacing: 6) {
      ContextSearchField(text: $query, prompt: "Find in Conversation",
      ...
      HStack(spacing: 8) {
        ...
        Spacer(minLength: 0)
        Button { move(true) } label: { Image(systemName: "chevron.up") }
          .help("Previous Matching Message").accessibilityLabel("Previous Matching Message")
          .disabled(count == 0)
        Button { move(false) } label: { Image(systemName: "chevron.down") }
