  ...
  var body: some View {
    ExactSourceComparisonSheetLayout(title: .init("Review Changes", locale: locale, bundle: .module),
      ...
      HStack {
        ...
        Spacer(minLength: 8)
        Button { controller.answer(requestID, allow: false) } label: { Text("Decline", bundle: .module) }
        Button { controller.answer(requestID, allow: true) } label: { Text("Allow Once", bundle: .module) }
