...
struct AgentChatToolEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Form {
                ...
                Section("Authentication and Environment") {
                    if edit.connection.kind == .remote {
                        TextField(
                            "Bearer Token Variable", text: $edit.connection.bearerTokenVariable,
            ...
            HStack {
                ...
                Button("Reload") {
                    operation = Task { @MainActor in
                        ...
                        if let fresh = await capabilities.reloadToolEdit(edit) {
                            ...
                            error = capabilities.toolConfigurationError ?? String(localized: "The tool configuration could not be reloaded.")
                        }
                    }
                ...
                .disabled(
                    operation != nil || !capabilities.canConfigureTools || edit.connection.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        ...
        .confirmationDialog("Remove Tool?", isPresented: $confirmsRemoval) {
            ...
            Text("Remove the configuration for \(edit.connection.name)? Its program and sign-in credentials will be kept.")
        }
    }
    ...
    private func save(removing: Bool = false) {
        ...
        operation = Task { @MainActor in
            ...
            if await capabilities.saveTool(edit, removing: removing) {
                ...
                error = capabilities.toolConfigurationError ?? String(localized: "The tool configuration could not be saved.")
            }
        }
    }
