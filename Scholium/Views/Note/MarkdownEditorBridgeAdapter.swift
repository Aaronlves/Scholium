...
enum EditorBridgeMessageDecoder {
    ...
    static func decode(_ body: Any) -> EditorBridgeMessage? {
        guard let object = body as? [String: Any],
            let type = boundedString(object["type"], maximumUTF8Bytes: 64)
        else {
            return nil
        }
        ...
        case "ready":
            guard hasOnlyKeys(object, additional: ["type"]),
                integer(object["protocolVersion"]) == markdownEditorProtocolVersion
            else {
                return nil
            }
            return .ready
        case "documentEnded":
            guard hasExactKeys(object, ["type", "editorReady"]),
                let editorReady = object["editorReady"] as? Bool
            else { return nil }
    ...
    private static func decodeSessionMessage(
        ...
        case "requestDocumentTitleRename":
            ...
            return .requestDocumentTitleRename(
                EditorDocumentTitleRenameMessage(
                    envelope: envelope,
                    requestID: requestID,
        ...
        case "requestDocumentFind":
            guard hasOnlyKeys(object, additional: ["type", "action"]),
                ...
                let action = DocumentFindShortcut(rawValue: rawAction)
            else { return nil }
            return .requestDocumentFind(
                EditorFindShortcutMessage(
                    envelope: envelope,
                    action: action
                ))
        case "linkCompletionQuery":
            ...
            return .linkCompletionQuery(
                EditorLinkCompletionQueryMessage(
                    envelope: envelope,
                    requestID: requestID,
                    query: query,
    ...
    private static func optionalFocusTarget(
        ...
        guard let raw = value as? String,
            let target = WindowDocumentFocusTarget(rawValue: raw)
        else {
            return nil
        }
        return .some(target)
