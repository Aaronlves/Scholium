    ...
    extension MarkdownEditorSession {
        ...
        func testingSimulateWebContentProcessTermination() -> Bool {
            guard let webView,
                let coordinator = webView.navigationDelegate as? MarkdownEditorWebView.Coordinator
            else {
                return false
            }
        ...
        func testingAccessibilitySnapshot() async throws -> TestingAccessibilitySnapshot {
            ...
            {
                let values =
                    try? await preview.callAsyncJavaScript(
                        """
                        return {
                            previewPopoverHidden: false,
                            previewTitle: document.querySelector('.scholium-preview-title')?.textContent || '',
                            previewNestedListCount: document.querySelectorAll('ul ul').length,
                            previewBlockquoteCount: document.querySelectorAll('blockquote').length,
                            previewCodeBlockCount: document.querySelectorAll('pre code.language-swift').length,
                            previewCalloutCount: document.querySelectorAll('.scholium-callout-state').length,
                            previewTableCount: document.querySelectorAll('table.scholium-table').length,
                            previewRenderedMathCount: document.querySelectorAll('.scholium-math-rendered').length
                        };
                        """, arguments: [:], in: nil, contentWorld: .page) as? [String: Any]
                if let values { snapshot.merge(values) { _, next in next } }
                rawResult = snapshot
            }
