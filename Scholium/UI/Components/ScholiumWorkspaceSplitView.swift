...
/// The single-Record workspace uses the same full-height native split-plane
/// grammar as the main Workspace. Plane backgrounds and their structural
/// boundary extend behind the transparent toolbar; the hosted reading and
/// evidence content remains constrained to AppKit's safe area.
struct ScholiumRecordDetailSplitView<Reading: View, Evidence: View>:
    NSViewControllerRepresentable
{
    let isEvidencePresented: Bool
    let reading: Reading
    let evidence: Evidence

    func makeNSViewController(context: Context) ->
        ScholiumRecordDetailSplitViewController<Reading, Evidence>
    {
        ScholiumRecordDetailSplitViewController(
    ...
    func updateNSViewController(
        _ controller: ScholiumRecordDetailSplitViewController<Reading, Evidence>,
        context: Context
    ) {
        controller.update(
            isEvidencePresented: isEvidencePresented,
...
@MainActor
final class ScholiumRecordDetailSplitViewController<Reading: View, Evidence: View>:
    NSSplitViewController
{
    private let readingHost: NSHostingController<Reading>
    private let evidenceHost: NSHostingController<Evidence>
    private let readingContainer: ScholiumSurfaceContainerViewController
    private let evidenceContainer: ScholiumSurfaceContainerViewController
    private var evidenceItem: NSSplitViewItem!
    private var requestedEvidencePresentation: Bool
    private var didApplyInitialPresentation = false

    init(
        isEvidencePresented: Bool,
        reading: Reading,
        evidence: Evidence
    ) {
        requestedEvidencePresentation = isEvidencePresented
        let readingHost = NSHostingController(rootView: reading)
        let evidenceHost = NSHostingController(rootView: evidence)
        self.readingHost = readingHost
        self.evidenceHost = evidenceHost
        readingContainer = ScholiumSurfaceContainerViewController(
            contentViewController: readingHost,
            backgroundRole: .document
        )
        evidenceContainer = ScholiumSurfaceContainerViewController(
            contentViewController: evidenceHost,
            backgroundRole: .apparatus,
            structuralDepthRole: .readingEvidenceBoundary
        )
        super.init(nibName: nil, bundle: nil)
    }
    ...
    @available(*, unavailable)
    required init?(coder: NSCoder) {
        fatalError("ScholiumRecordDetailSplitViewController is code-only")
    }

    override func viewDidLoad() {
        super.viewDidLoad()
        splitView.identifier = NSUserInterfaceItemIdentifier(
            "scholium.researchRecord.detailSplit"
        )
        splitView.isVertical = true
        splitView.dividerStyle = .thin

        let readingItem = NSSplitViewItem(viewController: readingContainer)
        readingItem.canCollapse = false
        readingItem.canCollapseFromWindowResize = false
        readingItem.allowsFullHeightLayout = true
        readingItem.titlebarSeparatorStyle = .line
        ...
        evidenceItem.titlebarSeparatorStyle = .line
        evidenceItem.isCollapsed = !requestedEvidencePresentation

        addSplitViewItem(readingItem)
        addSplitViewItem(evidenceItem)
    }
    ...
    override func viewWillAppear() {
        ...
        setEvidencePresented(requestedEvidencePresentation, animated: false)
    }
    ...
    func update(
        ...
        readingHost.rootView = reading
        evidenceHost.rootView = evidence
        requestedEvidencePresentation = isEvidencePresented
        guard didApplyInitialPresentation else { return }
        let shouldAnimate = view.window != nil
            && !NSWorkspace.shared.accessibilityDisplayShouldReduceMotion
        setEvidencePresented(isEvidencePresented, animated: shouldAnimate)
    }

    private func setEvidencePresented(_ isPresented: Bool, animated: Bool) {
        guard isViewLoaded,
              evidenceItem != nil,
              evidenceItem.isCollapsed == isPresented
        else { return }
        if animated {
