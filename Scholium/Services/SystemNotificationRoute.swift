...
typealias AgentChatNotificationSink = @MainActor (
...
enum SystemNotificationRoute: Codable, Hashable, Sendable {
  ...
  var triptychID: UUID {
    switch self { case .agentChange(let route): route.triptychID; case .chat(let route): route.triptychID }
  }
  var identifier: String {
    switch self { case .agentChange(let route): route.identifier; case .chat(let route): route.identifier }
  }
  var title: String {
    switch self {
