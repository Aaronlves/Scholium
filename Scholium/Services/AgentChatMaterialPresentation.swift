...
enum AgentChatLocalMaterialLabels {
    ...
    static func pageRanges(_ pages: [Int]) -> String {
        ...
        for page in pages {
            if let last = ranges.last, last.upperBound + 1 == page { ranges[ranges.count - 1] = last.lowerBound...page } else { ranges.append(page...page) }
        }
    ...
    static func summary(_ material: AgentChatLocalMaterial) -> String {
        ...
        case .pdf:
            material.pageImages.isEmpty
                ? ScholiumL10n.string("Extracted Text · \(material.pages.count) Pages")
