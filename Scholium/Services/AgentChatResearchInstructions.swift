...
enum AgentChatResearchInstructions {
    static func developer(triptychID: UUID) -> String {
        ...
        The current Triptych ID is \(triptychID.uuidString). Use the available Scholium tools for knowledge-base operations. When the task needs Zotero material, use the Codex host's Zotero capability for read-only library search, metadata and indexed attachment text. Do not start or configure a separate Zotero runtime, access Zotero's private database directly, or import and modify Zotero records from this Chat route. If the host capability is unavailable, explain that boundary. Report what was actually read: metadata, indexed full text, selected material and original-file access are distinct, and search metadata is not evidence that the original was read.
