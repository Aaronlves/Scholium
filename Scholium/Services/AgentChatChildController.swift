...
@MainActor final class AgentChatChildController: ObservableObject, Identifiable {
  ...
  init(childID: String, parentID: String, parentTitle: String,
    ...
    self.childID = childID; self.parentID = parentID; self.parentTitle = parentTitle; self.request = request
  ...
  func askParent() {
    ...
    isSending = true; receipt = nil
    coordination?.send(snapshot?.metadata.name) { [weak self] result in
      self?.isSending = false; self?.receipt = result
    }
  }
  ...
  func cancel() {
    ...
    connectionObservation?.cancel(); connectionObservation = nil
    parentObservation?.cancel(); parentObservation = nil
    isWorking = false
  }
  ...
  private func run(_ work: @escaping @MainActor () async throws -> Void) {
    ...
    operation = Task { [weak self] in
      ...
      catch CodexConnectionError.invalidMessage {
        if !Task.isCancelled { self?.error = .unverified }
      }
      catch CodexConnectionError.disconnected { self?.isDisconnected = true }
      catch CodexConnectionError.timedOut { if !Task.isCancelled { self?.error = .timedOut } }
      catch let failure as AgentChatChildFailure { if !Task.isCancelled { self?.error = failure } }
      catch { if !Task.isCancelled { self?.error = .runtime(error.localizedDescription) } }
    }
  }
...
enum AgentChatChildFailure: Error, Equatable {
  case unavailable, unverified, changedTurn, timedOut, runtime(String)
  func label(locale: Locale) -> String {
    switch self {
