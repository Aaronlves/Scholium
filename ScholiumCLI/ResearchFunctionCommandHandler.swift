    ...
    static func runFunction(
        ...
        guard let subcommand = arguments.first else {
            throw CLIError.usage(
                "Usage: scholium function <availability|prepare|select-methods|complete|cancel> ..."
            )
        }
        ...
        case "availability":
            guard let input = option("--from", in: arguments) else {
                throw CLIError.usage(
                    "Usage: scholium function availability --from <json|-> [--format json]"
                )
            }
            ...
            guard (option("--format", in: arguments) ?? "json") == "json" else {
                throw CLIError.usage("Function availability supports --format json.")
            }
        ...
        case "select-methods":
            guard let input = option("--from", in: arguments) else {
                throw CLIError.usage(
                    "Usage: scholium function select-methods --from <json|-> [--triptych <selector>] --format json|markdown"
                )
            }
            ...
            let handle = try await context.handle(for: assignment)
            let preparation = try await handle.research.selectFunctionMethods(submission)
            switch option("--format", in: arguments) ?? "markdown" {
            case "json":
                write(String(decoding: try encoder.encode(preparation), as: UTF8.self) + "\n")
            case "markdown":
            ...
            default:
                throw CLIError.usage(
                    "Function select-methods supports --format json or markdown."
                )
            }
